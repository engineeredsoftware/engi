...
export const V24_LOCAL_EXECUTOR_SCHEME = 'engi-local://';
...
function sidechainExecutor(payload) {
  ...
  const telemetry = buildTelemetry(payload, {
    ...
    affectedArtifactRefs: ['.engi/sidechain-execution-receipt.json']
  });
  return {
...
function githubExecutor(payload) {
  ...
  const branchName = priorBranch.branchName || String(payload.branchName || 'engi-v24-demo');
  return {
