...
| Transactions master-detail architecture | master-detail risked reading as generic run detail rather than a rich Bitcode transaction workspace | V26 must make master detail concretely a searchable/filterable Bitcode transactions table with transaction detail as detail | `ApplicationTransactionsTable.tsx`, `application-transactions.ts`, `BitcodeTransactionsTable.tsx`, `BitcodeTransactionsOverview.tsx`, `BitcodeTransactionsFilterBar.tsx`, `BitcodeTransactionsDataTable.tsx`, `bitcode-transaction-types.ts`, and `applicationTransactions.test.ts` now establish a typed, SRP-aligned transactions master surface inside `/application`, with free-text, status, ownership, repository, participant, proof-posture, and sort filtering plus application-owned central detail | substantially advanced |
| Route-owned transaction query state | transaction selection and rich master-table filter posture still risked living only in component-local state | V26 must make transaction selection and filter posture shareable and route-owned inside `/application` | `application-transaction-query.ts`, `ApplicationPageClient.tsx`, `ApplicationTransactionWorkspace.tsx`, `ApplicationTransactionsTable.tsx`, `BitcodeTransactionsFilterBar.tsx`, and `applicationTransactionQuery.test.ts` now keep transaction selection and rich master filters in application query state while preserving inbound `runId` compatibility | substantially advanced |
| Route-owned transaction detail interaction | selected-transaction focus and closure actions still risked living as local detail-surface behavior rather than shareable application state | V26 must make transaction-detail focus and closure follow-through application-owned inside `/application` | `application-transaction-query.ts`, `ApplicationTransactionDetailActionBar.tsx`, `ApplicationTransactionDetailSurface.tsx`, and `applicationTransactionQuery.test.ts` now keep detail focus in route query state and expose closure rerun plus detail refresh through the shell bridge | substantially advanced |
...
| Shared shell bridge provider | second-gate application carriers were independently polling shell state and control availability | V26 second-gate must centralize mounted-shell state polling and refresh/control orchestration behind one reusable bridge | `application-shell-bridge.tsx` and `applicationShellBridge.test.tsx` now provide one shared provider/hook for shell snapshot and control reuse across `/application` carriers | substantially advanced |
