...
| Two-action architecture | current source carries depositing/needing semantics, but the application frame does not yet fully present them as the two top-level Bitcode actions | V26 must lock `give` and `need` as the two main operator actions inside the master-detail experience | `ApplicationExperienceFrame.tsx`, `ApplicationCommandDeck.tsx`, `ApplicationSupplySelectionPanel.tsx`, `ApplicationCoreNativeSections.tsx`, and `ApplicationGiveNeedWorkbench.tsx` now make give and need explicit in the live application frame, with shell-snapshot and shell-control bridges feeding application-owned command, intake, and action detail | substantially advanced |
