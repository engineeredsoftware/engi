...
| Orbital route ownership | current source now has non-legacy `/orbitals/users`, `/orbitals/connects`, `/orbitals/credits`, and `/orbitals/models` pages backed by active app-route files rather than only a pathname fallback inside the orbital client, and those routes now default to account/login-first posture instead of signup-first posture | V26 requires account/onboarding orbital deep links to be application-owned | active app-route files exist for the orbital step family, `/orbitals` resolves into that family, and direct routes now open with settings-oriented posture | partially implemented |
| Orbital connects-pane repair | current source replaced the broken `OrbitalsConnectsPane` self-reexport with a real app-owned connects pane using current VCS/auth surfaces | V26 requires recovered orbital steps to run on current component ownership rather than broken demo-era stubs | connects step renders through current app components and keeps GitHub as the onboarding gate | partially implemented |
| Orbital settings rehabilitation | current source now preserves the ringed orbital overlay shell while making signed-in orbitals behave like Bitcode settings navigation instead of sequential onboarding-only gating, including restored explicit save actions inside settings panes that previously depended on hidden onboarding submit hooks | V26 requires orbital to become the real settings surface for user/account areas without discarding the established ring-overlay experience | orbital root, headers, step indicator, and settings panes stay ring-based while signed-in users can navigate and save settings directly | partially implemented |
| Bitcode rename follow-on cleanup in touched settings surfaces | current source now updates touched orbital/settings copy away from lingering ENGI-facing strings such as legacy build email/domain display and organization security examples like `engi_*` API key placeholders | V26 must keep tightening Bitcode-facing naming wherever active application-facing surfaces are being rehabilitated | touched user-facing orbital/settings surfaces no longer present legacy ENGI naming unless explicitly compatibility-bound | partially implemented |
...
| API/runtime re-homing | current standalone demo server still owns major Bitcode route composition, but current source now also includes a non-legacy `uapi/app/api/auth/unlink/route.ts` backed by `@engi/api` rather than a demo-local route owner | V26 must re-home Bitcode API composition into `packages/api` and application-owned route surfaces | standalone demo server is no longer the primary owner of Bitcode route behavior | partially implemented |
| Application-owned auth account management | current source now exposes connected-account settings inside the orbital users surface and routes unlink behavior through an app-owned auth endpoint | V26 requires basic account-provider management to live inside the application-owned settings surface rather than demo-only or client-only flows | supported providers can be connected/disconnected inside the active application settings surface | partially implemented |
| Auth and wallet productionization | current repo has MetaMask and auth groundwork, and current source now narrows the active auth surface to email-code plus GitHub/Google while explicitly staging wallet connection instead of presenting unsupported providers as live | V26 must refine auth and connecting-wallet posture to production readiness | wallet verification and production auth flow are explicit, tested, and app-native | partially implemented |
...
| Demo-to-app rule | Bitcode becomes a first-class application page | partially implemented |
| Embedded-demo removal | homepage no longer depends on an embedded demo | implemented |
| Orbital route ownership | `/orbitals/*` resolves through active app routes rather than fallback-only client parsing | partially implemented |
| Orbital settings rehabilitation | signed-in orbital usage behaves like a settings surface while preserving the ringed overlay shell | partially implemented |
...
| Wallet/auth hardening | production wallet connection and auth posture are explicit | partially implemented |
