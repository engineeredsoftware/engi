...
| Second-gate application experience | the preserved shell still uses the carried-forward shell DOM/CSS/JS contract | V26 second-gate must replace preserved shell implementation with more native application-facing composition, preserve late-Engi design-system atmosphere, harden external interfacings for stable readiness, and follow the explicit second-gate design contract | application-owned command deck, live summary strip, body atlas, core operating/deposit/need/fit native cards, workspace rail, central run-detail workspace, mock deliverable-reading reuse, fullscreen conversations mounting, and app-route conversation interfaces now exist around the preserved shell, with live browser verification showing route-local controls, summary state, section-jump body cards, and the first native core body layer stay synchronized to the preserved shell | substantially advanced |
