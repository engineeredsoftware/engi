...
| Master-detail substructure architecture | runs, deliverables, proofs, and history still risk reading like adjacent imported panels instead of one inner Bitcode structure | V26 must make runs, deliverables, proofs, and history explicit as the four master-detail substructures inside `/application` | `ApplicationRunWorkspace.tsx` now exposes those four substructures directly in the live application workspace, with inward run detail, deliverable panels, proof posture, and history/accounting state reading as one Bitcode carrier | substantially advanced |
| Second-gate application experience | the preserved shell still uses the carried-forward shell DOM/CSS/JS contract | V26 second-gate must replace preserved shell implementation with more native application-facing composition, preserve late-Engi design-system atmosphere, harden external interfacings for stable readiness, and follow the explicit second-gate design contract | application-owned architecture frame, command deck, live summary strip, body atlas, core operating/deposit/need/fit native cards, closure-side verification/artifact/settlement/ledger native cards, workspace rail, central run-detail workspace, explicit master-detail substructure cards, mock deliverable-reading reuse, fullscreen conversations mounting, and app-route conversation interfaces now exist around the preserved shell, with live browser verification showing route-local controls, give/need framing, section-jump body cards, and both native core and native closure layers stay synchronized to the preserved shell after `Make Bitcode branch` | substantially advanced |
