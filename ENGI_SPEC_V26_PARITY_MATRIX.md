...
| Master-detail substructure architecture | transactions, deliverables, proofs, and history still risk reading like adjacent imported panels instead of one inner Bitcode structure | V26 must make transactions, deliverables, proofs, and history explicit as the four master-detail substructures inside `/application` | `ApplicationRunWorkspace.tsx`, `ApplicationTransactionsTable.tsx`, `ApplicationRunDetailSurface.tsx`, `ApplicationTransactionDetailHero.tsx`, `ApplicationTransactionIdentityCard.tsx`, `ApplicationTransactionClosureCard.tsx`, `ApplicationRunActivitySurface.tsx`, `application-transactions.ts`, `application-run-detail.ts`, `application-transaction-detail.ts`, and `application-run-activity.ts` now expose those four substructures directly in the live application workspace, with normalized transaction detail payloads and retained execution/log carriers feeding one Bitcode detail surface | substantially advanced |
