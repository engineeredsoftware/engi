...
| Master-detail substructure architecture | runs, deliverables, proofs, and history still risk reading like adjacent imported panels instead of one inner Bitcode structure | V26 must make runs, deliverables, proofs, and history explicit as the four master-detail substructures inside `/application` | `ApplicationRunWorkspace.tsx`, `ApplicationRunDetailSurface.tsx`, `ApplicationRunActivitySurface.tsx`, `application-run-detail.ts`, and `application-run-activity.ts` now expose those four substructures directly in the live application workspace, with normalized selected-run history payloads and retained execution/log carriers feeding one Bitcode detail surface | substantially advanced |
