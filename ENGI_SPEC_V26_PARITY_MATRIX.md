...
| Demonstration-to-application integration | current homepage embeds the Bitcode demonstration and links to a standalone surface | V26 requires a first-class application page for the Bitcode operator experience | app-native route exists and homepage no longer depends on an embedded or standalone demo | draft-target |
| Embedded marketing demo removal | current marketing page includes `MarketingEmbeddedDemoSection` and standalone-launch CTAs | V26 removes embedded-demo dependency from the target posture | homepage no longer contains the embedded-demo section or standalone-primary CTA posture | draft-target |
