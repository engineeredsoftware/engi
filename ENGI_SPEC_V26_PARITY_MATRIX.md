...
| Master-detail substructure architecture | runs, deliverables, proofs, and history still risk reading like adjacent imported panels instead of one inner Bitcode structure | V26 must make runs, deliverables, proofs, and history explicit as the four master-detail substructures inside `/application` | `ApplicationRunWorkspace.tsx`, `ApplicationRunDetailSurface.tsx`, and `application-run-detail.ts` now expose those four substructures directly in the live application workspace, with normalized selected-run history payloads feeding deliverable panels, proof posture, and history/accounting state inside one Bitcode carrier | substantially advanced |
| Second-gate application experience | the preserved shell still uses the carried-forward shell DOM/CSS/JS contract | V26 second-gate must replace preserved shell implementation with more native application-facing composition, preserve late-Engi design-system atmosphere, harden external interfacings for stable readiness, and follow the explicit second-gate design contract | application-owned architecture frame, command deck, live summary strip, body atlas, core operating/deposit/need/fit native cards, closure-side verification/artifact/settlement/ledger native cards, workspace rail, central run-detail workspace, normalized selected-run detail with live/mocked deliverable panels, fullscreen conversations mounting, and app-route conversation interfaces now exist around the preserved shell, with live browser verification showing route-local controls, give/need framing, section-jump body cards, and both native core and native closure layers stay synchronized to the preserved shell after `Make Bitcode branch` | substantially advanced |
...
| Runs, pipelines, and deliverable meaning | current executions/deliverables systems still carry older pipeline-specific meaning | V26 must totalize runs/pipelines, redefine deliverable meaning under Bitcode operations, and port the strongest master-detail patterns inward to `/application` | a central application-owned run workspace now carries selected-run detail and mock deliverable-reading panels inside `/application`, while deeper semantic convergence remains open | substantially advanced |
