...
| Transactions master-detail architecture | master-detail risked reading as generic run detail rather than a rich Bitcode transaction workspace | V26 must make master detail concretely a searchable/filterable Bitcode transactions table with transaction detail as detail | `ApplicationTransactionsTable.tsx`, `application-transactions.ts`, `BitcodeTransactionsTable.tsx`, and `applicationTransactions.test.ts` now establish a typed transactions master surface inside `/application`, while central detail remains application-owned | substantially advanced |
...
| Second-gate application experience | the preserved shell still uses the carried-forward shell DOM/CSS/JS contract | V26 second-gate must replace preserved shell implementation with more native application-facing composition, preserve late-Engi design-system atmosphere, harden external interfacings for stable readiness, and follow the explicit second-gate design contract | application-owned architecture frame, command deck, live summary strip, body atlas, core operating/deposit/need/fit native cards, semantic-snapshot-driven give/need action detail, route-local supply-selection controls, route-local native deposit submission, closure-side verification/artifact/settlement/ledger native cards, workspace rail, central run-detail workspace, normalized selected-run detail with live/mocked deliverable panels, application-owned run activity/log surfaces, fullscreen conversations mounting, and app-route conversation interfaces now exist around the preserved shell | substantially advanced |
