...
# 2) Ensure referenced PromptParts in AssetPack prompts have 0.50.0 and no fallback text
node - << 'NODE'
...
if(bad>0||not50>0){console.error(`AssetPack quality check failed: fallback=${bad} non-50.0=${not50}`);process.exit(1);} else {console.log('AssetPack quality OK');}
NODE
...
# 4) Ensure AssetPack prompts have @doc-comment-developing-promptdevelopment
node - << 'NODE'
const fs=require('fs');const path=require('path');
...
if(missing>0){console.error(`AssetPack prompt doc coverage failed: missing=${missing}`);process.exit(1);} else {console.log('AssetPack prompt doc coverage OK');}
NODE

echo "All AssetPack prompt quality checks passed."
