...
const DEFAULT_RECEIPT_DIR = '.bitcode/pipeline-harness-runs/v40-rehearsal-receipts';
...
function parseArgs(argv) {
  ...
  if (!V40_REHEARSAL_LANES.includes(args.lane)) {
    throw new Error(`Unsupported rehearsal lane ${args.lane}. Expected one of ${V40_REHEARSAL_LANES.join(', ')}.`);
  }
...
function printHelp() {
  process.stdout.write(
    [
      'Usage: node scripts/rehearse-v40-local-staging-testnet.mjs --lane <local|staging-testnet> [--dry-run|--execute] [--json] [--write-receipt]',
      '',
      'Builds a source-safe operator receipt for the V40 local/staging-testnet Reading rehearsal lane.',
      'Live execution requires BITCODE_V40_REHEARSAL_EXECUTE=1 and delegates to the Vercel Sandbox AssetPack harness.',
    ].join('\n'),
  );
  process.stdout.write('\n');
}
...
function buildReceipt(args, env = process.env) {
  ...
  const command = {
    ...
    liveExecutionOptInSatisfied: env.BITCODE_V40_REHEARSAL_EXECUTE === '1',
  };
  const withoutRoot = {
    schema: 'bitcode.v40.localStagingRehearsal.operatorReceipt',
    version: 'V40',
    currentTarget: 'V39',
    laneId: args.lane,
    laneClass: args.lane === 'staging-testnet' ? 'staging-testnet-real-inference' : 'local-operator-rehearsal',
...
function runLiveHarness(receipt) {
  if (!receipt.ready) {
    throw new Error(`Cannot execute V40 rehearsal; missing families: ${receipt.missingEnvironmentFamilies.join(', ')}`);
  }
  if (!receipt.command.liveExecutionOptInSatisfied) {
    throw new Error('Set BITCODE_V40_REHEARSAL_EXECUTE=1 before live rehearsal execution.');
  }
...
function main() {
  ...
  process.stdout.write(
    [
      `V40 ${receipt.laneId} rehearsal receipt ${receipt.ready ? 'ready' : 'not ready'}.`,
      `Receipt root: ${receipt.receiptRoot}`,
      receiptPath ? `Receipt path: ${receiptPath}` : 'Receipt not written; pass --write-receipt to persist it.',
      receipt.missingEnvironmentFamilies.length
        ? `Missing families: ${receipt.missingEnvironmentFamilies.join(', ')}`
        : 'No required environment families missing.',
    ].join('\n'),
  );
  process.stdout.write('\n');
}
