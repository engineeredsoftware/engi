...
find packages/pipelines/asset-pack -name "*.ts" -exec awk '
  /^import .* from .zod.;/ { 
    if (!seen_zod) { 
      print; 
      seen_zod = 1 
...
MALFORMED=$(grep -r "promptpart_.*';" packages/pipelines/asset-pack --include="*.ts" | wc -l)
echo "Found $MALFORMED malformed promptpart imports"

WRONG_PROMPT=$(grep -r "import { Prompt } from '@bitcode/prompts' packages/pipelines/asset-pack --include="*.ts" | wc -l)
echo "Found $WRONG_PROMPT wrong Prompt import paths"

WRONG_CREATE=$(grep -r "import { createPromptPart } from '@bitcode/prompts' packages/pipelines/asset-pack --include="*.ts" | wc -l)
