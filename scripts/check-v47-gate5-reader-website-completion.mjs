...
function printHelp() {
  process.stdout.write(
    [
      'Usage: node scripts/check-v47-gate5-reader-website-completion.mjs [--skip-branch-check] [--skip-package-tests] [--skip-uapi-tests] [--repo-root <path>]',
      '',
      'Checks V47 Gate 5 reader website completion, generated artifact freshness, fit measurement review, settlement/rights/delivery ordering, workflows, and focused tests.',
    ].join('\n'),
  );
  process.stdout.write('\n');
}
...
function main() {
  ...
  for (const relativePath of [
    ...
    assertCheck(failures, exists(root, relativePath), `Missing required V47 Gate 5 file: ${relativePath}`);
  }

  const artifact = buildV47ReaderWebsiteCompletion({ repoRoot: root });
  assertCheck(failures, artifact.passed, `V47 reader website completion predicates failed: ${artifact.coverage.failedPredicateIds.join(', ')}`);
  assertCheck(failures, artifact.coverage.readRequestInitiationComplete === true, 'Read request initiation must be complete.');
  assertCheck(failures, artifact.coverage.needReviewAcceptanceComplete === true, 'Need review and acceptance must be complete.');
  assertCheck(failures, artifact.coverage.fitMeasurementReviewComplete === true, 'Fit measurement review must be complete.');
  assertCheck(failures, artifact.coverage.quoteBeforeSettlementComplete === true, 'Quote-before-settlement must be complete.');
  assertCheck(
    failures,
    artifact.coverage.settlementFinalityRightsDeliveryComplete === true,
    'Settlement, finality, rights, and delivery readback must be complete.',
  );
  ...
  assertCheck(
    failures,
    exists(root, V47_READER_WEBSITE_COMPLETION_ARTIFACT_PATH) &&
      read(root, V47_READER_WEBSITE_COMPLETION_ARTIFACT_PATH) === serialized,
    `${V47_READER_WEBSITE_COMPLETION_ARTIFACT_PATH} must be generated and current.`,
  );

  const packageJson = read(root, 'package.json');
  const gateWorkflow = read(root, '.github/workflows/bitcode-gate-quality.yml');
  const canonWorkflow = read(root, '.github/workflows/bitcode-canon-quality.yml');
  assertCheck(failures, packageJson.includes('"generate:v47-reader-website-completion"'), 'package.json must expose generate:v47-reader-website-completion.');
  assertCheck(failures, packageJson.includes('"check:v47-reader-website-completion"'), 'package.json must expose check:v47-reader-website-completion.');
  assertCheck(failures, packageJson.includes('"check:v47-gate5"'), 'package.json must expose check:v47-gate5.');
  assertCheck(failures, gateWorkflow.includes('check-v47-gate5-reader-website-completion.mjs'), 'Gate workflow must run V47 Gate 5 checker.');
  assertCheck(failures, canonWorkflow.includes('check-v47-gate5-reader-website-completion.mjs'), 'Canon workflow must run V47 Gate 5 checker.');
  ...
  if (failures.length > 0) {
    process.stderr.write('V47 Gate 5 reader website completion check failed:\n');
    for (const failure of failures.filter(Boolean)) process.stderr.write(`- ${failure}\n`);
    process.exitCode = 1;
    return;
  }

  process.stdout.write('V47 Gate 5 reader website completion check passed.\n');
}
