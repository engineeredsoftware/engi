...
## 8.7 Evaluators, evaluations, and typed measurements

ENGI distinguishes:
- an **evaluator**: a declared mechanism/contract that can produce one or more typed measurements,
- an **evaluation**: one execution of that evaluator on specific inputs at a specific point in the pipeline,
- a **measurement**: the typed output produced by that evaluation.

```ts
type MeasurementMode = 'static' | 'inferred' | 'hybrid'

type EvaluatorSpec = {
  evaluatorId: string                 // canonical evaluator identifier
  evaluatorVersion: string            // contract version for replay/debugging
  mode: MeasurementMode               // whether the evaluator is static, inferred, or hybrid
  measurementTypes: string[]          // typed measurements this evaluator may produce
  executesDuring: string              // pipeline phase where it runs
}

type EvaluationRun<TInput, TOutput> = {
  evaluationId: string                // unique execution id
  evaluator: EvaluatorSpec            // evaluator contract that produced this run
  input: TInput                       // canonical input object for this execution
  output: TOutput                     // typed measurement(s) produced by this execution
  computedAt: string                  // pipeline time/place of execution
  provenance: MeasurementProvenance[] // tool/prompt/version/evidence trace
}
```

Rules:
- a static tool/program MAY satisfy the evaluator contract and produce a typed measurement,
- a prompted inference MAY satisfy the evaluator contract and produce a typed measurement,
- a hybrid evaluator MAY combine deterministic extraction and inference to produce a typed measurement,
- downstream systems MUST consume typed measurements, not ad hoc prose judgments.

## Review annotation
- This section unifies evaluators, evaluations, and measurements so that static tooling and prompted inference fit the same typed model.
...
# 12. Whole-asset need scoring

## 12.1 Overview

**Responsibility:** aggregate unit-level, asset-level, static, and inferred measurements into a single whole-asset need-resolution view.

**Inputs:** candidate asset, candidate units, static measurements, inferred measurements, benchmark target, penalties.

**Outputs:** a whole-asset need score decomposition that later feeds final ranking.

**Hand-off:** whole-asset need scoring provides the holistic asset view consumed by final ranking and later share computation.

## 12.2 Type

```ts
type WholeAssetNeedScore = {
  assetId: string
  unitMeasurements: {
    unitId: string
  ...
  assetLevelMeasurements: {
    staticMeasurements: MeasurementProvenance[]
    inferredMeasurements: MeasurementProvenance[]
  }
  aggregatedNeedScore: number
}
```

## 12.3 Aggregation rules

Whole-asset need scoring MUST:
- aggregate unit-level signals into an asset-level view,
- preserve the distinction between static and inferred measurements,
- retain enough traceability that the selected asset can be explained in terms of units, evidence, and evaluator outputs,
- feed later ranking and share computation without losing measurement provenance.

## Review annotation
- This section makes whole-asset need scoring explicit rather than leaving it implicit inside later ranking composition.
...
# 15. Benchmark impact likelihood
...
# 19. Candidate verification result and use tier
...
# 20. Expensive LLM evaluation roles
...
# 24. Raw asset share computation
...
# 25. Settlement and journal diff
...
# 27. Telemetry, debugging, and implementation reliability
...
# 30. Sensitive data, identity, and cross-cutting proof model
...
## B2.1 Evaluator, evaluation, and measurement relationship

In ENGI, a prompted evaluator is one valid implementation of the general evaluator contract. That means a prompted evaluator may produce a typed measurement, just like a static tool/program or hybrid evaluator.

The key distinction is:
- evaluator = contract/mechanism
- evaluation = one run of that mechanism
- measurement = typed output produced by that run

This appendix covers only the prompted-inference subset of evaluators, but its outputs are still typed measurements and must fit the same system-wide measurement model.

## B2.2 Inference design rules
...
## B2.3 Universal evaluator envelope
...
## B2.4 Static prompt template vs context injectables
...
## B2.5 Canonical context injectable catalog
...
Field documentation:
- `taskSemanticFit`: inferred during ranking need-match evaluation from the measured need descriptor, benchmark target, and candidate content
- `failureModeFit`: inferred during ranking need-match evaluation from failing-case evidence and candidate content
- `constraintFit`: inferred during ranking need-match evaluation from need constraints plus candidate content and static measurements
- `artifactKindFit`: inferred during ranking need-match evaluation when static artifact-kind comparison alone is insufficient
- `notes`: non-canonical explanatory support for debugging and review

## B2.7 Benchmark impact evaluator
...
Field documentation:
- `likelyImprovesFailingCases`: inferred during benchmark-impact evaluation from canonical failing cases plus candidate content
- `likelyImprovesWeakDimensions`: inferred during benchmark-impact evaluation from weak-dimension evidence plus candidate content
- `likelyGeneralizesToRepoContext`: inferred during benchmark-impact evaluation from buyer repo context, touched paths, symbols, and candidate content
- `notes`: non-canonical explanatory support for debugging and review

## B2.8 Actionability evaluator
...
Field documentation:
- `remediationSpecificity`: inferred during actionability evaluation from the candidate's corrective-action detail against the measured need
- `implementationSpecificity`: inferred or confirmed during actionability evaluation from code/config/change specificity
- `operationalUsability`: inferred during actionability evaluation from workflow readiness, rerun guidance, and branch-mode relevance
- `notes`: non-canonical explanatory support for debugging and review

## B2.9 Technical coherence evaluator
...
## B2.10 Provenance/verification evaluator
...
## B2.11 Selection-consistency evaluator (optional)
...
## B2.12 Parsable completion contract
...
## B2.13 Inference hand-off rules
...
## B2.14 Inference telemetry requirements
...
This appendix gives a compact but complete specification of the major ENGI measurements. It is shorter than the main body, but each measurement entry is still complete enough to implement or audit.

Every measurement entry states:
- purpose,
- computation inputs,
- where/time in the pipeline it is computed,
- whether it is static, inferred, or hybrid,
- output type,
- algorithm.

## B3.1 Task semantic fit

- **Purpose:** measure how strongly the candidate addresses the buyer task as a whole
- **Computed during:** ranking / need-match stage
- **Inputs:** need descriptor task, benchmark target, candidate content, task-vector recall support
- **Mode:** hybrid
- **Output:** `NeedMatchScore.taskSemanticFit`

Algorithm:
1. gather task-relevant static evidence,
2. run task-vector recall and any supporting lexical/symbol evidence,
3. if needed, run the need-match evaluator,
4. emit a bounded score in `[0,1]`.

## B3.2 Failure-mode fit

- **Purpose:** measure whether the candidate addresses the exact failure class, not just a nearby domain
- **Computed during:** ranking / need-match stage
- **Inputs:** failing cases, weak dimensions, candidate content, failure-mode vector evidence
- **Mode:** hybrid
- **Output:** `NeedMatchScore.failureModeFit`

Algorithm:
1. normalize failing-case evidence,
2. compare candidate against failure-mode evidence,
3. run evaluator support where deterministic comparison is insufficient,
4. emit a bounded score in `[0,1]`.

## B3.3 Symbol fit

- **Purpose:** measure repository-symbol alignment
- **Computed during:** ranking / need-match stage
- **Inputs:** extracted repo symbols, extracted candidate symbols
- **Mode:** primarily static
- **Output:** `NeedMatchScore.symbolFit`

Algorithm:
1. extract canonical symbol sets,
2. compare for overlap/weighted alignment,
3. normalize into `[0,1]`.

## B3.4 Path fit

- **Purpose:** measure repository-path/subsystem alignment without over-penalizing non-code artifacts
- **Computed during:** ranking / need-match stage
- **Inputs:** touched paths, candidate source paths, candidate mentioned paths
- **Mode:** hybrid
- **Output:** `NeedMatchScore.pathFit`

Algorithm:
1. compute source-path precision,
2. compute mentioned-path support,
3. compute subsystem alignment,
4. combine through `computeFinalPathFit`.

## B3.5 Stack fit

- **Purpose:** measure stack/framework/dependency alignment
- **Computed during:** ranking / need-match stage
- **Inputs:** stack hints, imports, dependencies, config keys, candidate declarations
- **Mode:** hybrid
- **Output:** `NeedMatchScore.stackFit`

Algorithm:
1. normalize stack terms,
2. compare candidate stack evidence to need stack evidence,
3. emit a bounded score in `[0,1]`.

## B3.6 Constraint fit

- **Purpose:** measure whether the candidate preserves the need's required constraints
- **Computed during:** ranking / need-match stage
- **Inputs:** need constraints, extracted candidate constraints, proof-bearing materials, evaluator support
- **Mode:** hybrid
- **Output:** `NeedMatchScore.constraintFit`

Algorithm:
1. identify explicit constraints in the need,
2. identify candidate evidence supporting or violating those constraints,
3. use evaluator support where deterministic interpretation is insufficient,
4. emit a bounded score in `[0,1]`.

## B3.7 Artifact-kind fit

- **Purpose:** measure whether the content form is appropriate for the need
- **Computed during:** ranking / need-match stage
- **Inputs:** target artifact kinds, candidate artifact kind
- **Mode:** mostly static
- **Output:** `NeedMatchScore.artifactKindFit`

Algorithm:
1. compare target and candidate artifact kinds,
2. use evaluator support only when classification is ambiguous,
3. emit a bounded score in `[0,1]`.

## B3.8 Lexical support

- **Purpose:** provide bounded lexical evidence without dominating need match
- **Computed during:** ranking / need-match stage
- **Inputs:** BM25/token overlap, keyphrase matches
- **Mode:** static
- **Output:** `NeedMatchScore.lexicalSupport`

Algorithm:
1. compute lexical relevance using declared lexical tooling,
2. normalize into `[0,1]`.

## B3.9 Benchmark-impact measurements

### `likelyImprovesFailingCases`
- **Computed during:** ranking / benchmark-impact stage
- **Inputs:** canonical failing cases, candidate content
- **Mode:** hybrid

### `likelyImprovesWeakDimensions`
- **Computed during:** ranking / benchmark-impact stage
- **Inputs:** weak-dimension evidence, candidate content
- **Mode:** hybrid

### `likelyGeneralizesToRepoContext`
- **Computed during:** ranking / benchmark-impact stage
- **Inputs:** repo context, touched paths, symbols, candidate content
- **Mode:** hybrid

All three are combined through `computeFinalBenchmarkImpact`.

## B3.10 Actionability measurements

### `remediationSpecificity`
- **Computed during:** ranking / actionability stage
- **Inputs:** measured need, candidate corrective-action detail
- **Mode:** hybrid

### `implementationSpecificity`
- **Computed during:** ranking / actionability stage
- **Inputs:** code/config/change specificity, symbols, paths, hunks, config keys
- **Mode:** primarily static

### `operationalUsability`
- **Computed during:** ranking / actionability stage
- **Inputs:** rerun guidance, workflow readiness, branch-mode relevance
- **Mode:** hybrid

All three are combined through `computeFinalActionability`.

## B3.11 Verification sufficiency

- **Purpose:** determine whether the candidate has enough evidence to support branch use
- **Computed during:** verification determinisms stage
- **Inputs:** tests, typecheck, static analysis, benchmark evidence, repro steps, pinned environment
- **Mode:** hybrid
- **Output:** `VerificationSufficiency`

Algorithm:
1. gather deterministic evidence booleans,
2. compute a bounded sufficiency score,
3. store `scoreTrace`,
4. derive `recommendedUseTier`.

## B3.12 Whole-asset need score

- **Purpose:** aggregate unit-level and asset-level measurements into a holistic asset view
- **Computed during:** ranking after unit-level measurement collection
- **Inputs:** unit measurements, static measurements, inferred measurements, penalties
- **Mode:** hybrid
- **Output:** `WholeAssetNeedScore`

Algorithm:
1. collect unit-level measurements,
2. aggregate into asset-level need-resolution view,
3. retain traceability to unit refs and provenance,
4. hand off to final ranking.

## B3.13 Final ranking score

- **Purpose:** produce the final need-resolution score for the candidate
- **Computed during:** ranking finalization
- **Inputs:** need match, benchmark impact, actionability, penalties
- **Mode:** hybrid aggregation over prior measurements
- **Output:** final ranking score

Algorithm:
1. compute `computeFinalNeedMatch`,
2. compute `computeFinalBenchmarkImpact`,
3. compute `computeFinalActionability`,
4. subtract bounded penalty mass,
5. emit telemetry for all component values and final score.

## B3.14 Use-tier derivation

- **Purpose:** convert verification determinisms into downstream eligibility
- **Computed during:** verification/use-tier stage
- **Inputs:** issuance verification, provenance verification, verification sufficiency, issuer policy status
- **Mode:** deterministic policy logic over prior measurements
- **Output:** `CandidateUseTier`

Algorithm:
1. reject on computed hard failures,
2. map verification sufficiency to analysis/context/patch eligibility,
3. optionally upgrade to settlement eligibility only under declared conditions.

## B3.15 Raw share computation

- **Purpose:** normalize contribution across settlement-relevant assets
- **Computed during:** settlement preparation
- **Inputs:** need descriptor, final settlement-eligible bundle
- **Mode:** deterministic over prior whole-asset measurements
- **Output:** `AssetShareRaw[]`

Algorithm:
1. compute whole-bundle score,
2. compute leave-one-asset-out marginal contribution,
3. normalize to 10000 basis points exactly.

## B3.16 Settled share computation (default v1 mode)

- **Purpose:** define settlement shares used in accounting
- **Computed during:** settlement preparation
- **Inputs:** raw shares
- **Mode:** deterministic
- **Output:** `AssetShareSettled[]`

Algorithm:
1. copy raw shares into settled shares,
2. require `settled == raw` in default v1 mode.

## B3.17 Journal and accounting measurements

### Micro-unit allocation
- **Computed during:** settlement
- **Inputs:** metered micro-units, settled shares
- **Mode:** deterministic
- **Output:** exact micro-unit allocations

### Journal invariants
- **Computed during:** settlement proofing
- **Inputs:** journal entries, before/after balances, roots, asset pack lock
- **Mode:** deterministic
- **Output:** `JournalDiff.invariants` + `SettlementProof`

## Review annotation
- This appendix is intentionally abridged but complete: each major measurement says what it measures, where it is computed, what it consumes, what type it emits, and how it is algorithmically formed.
...
15. Evaluator / measurement model
   - Are evaluators, evaluations, and typed measurements now sufficiently unified?

16. Branch modes
   - Is the distinction between context branch and patch branch operationally precise enough?

17. Measurement appendix
   - Does the abridged measurement appendix cover each major measurement completely enough?

18. One-liner / core contract
