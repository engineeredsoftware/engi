...
- `BitcodeActionPillRow.tsx` plus `bitcodeActionPillRow.test.tsx` and `BitcodePayloadDetailCard.tsx` plus `bitcodePayloadDetailCard.test.tsx` now centralize the repeated payload-inspector-plus-actions shell used by transaction identity, closure, proofs, and history, so those cards share one hardened execution-level carrier instead of reauthoring the same button-row posture.
