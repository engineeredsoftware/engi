...
- `BitcodeDetailRowList.tsx`, `BitcodeMetricGrid.tsx`, `BitcodeChipCloud.tsx`, `bitcodeDetailRowList.test.tsx`, and `bitcodeMetricGrid.test.tsx` now move selected-transaction rows, metric grids, and artifact chips onto reusable base carriers, and `ApplicationTransactionClosureCard.tsx` now exposes the same raw JSON payload inspection posture as identity, proofs, and history.
