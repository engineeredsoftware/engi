...
| Closer-to-real workflow realism | explicit integration tests now cover repo-authenticated deposit composition, normalization/projection behavior, privacy-boundary disclosure vs replay visibility, restrictive unsafe-patch verification/materialization behavior, prompt/inference downstream consumer closure, static-vs-verification receipt-domain separation, selection/auth/disclosure visibility closure, and settlement zero-credit exactness | V17 should continue growing those flows toward fuller production-like coverage and failure-mode depth | workflow integration tests cover deposit -> branch -> proof/artifact/settlement/state behavior across multiple scenario classes, consequence classes, and family-member semantics | strongly advanced |
| Browser E2E breadth | browser tests now cover targeted settlement, normalization/source-to-shares, privacy-boundary identity/auth plus proof/disclosure surfaces, restrictive raw proof inspection with family/member/theorem visibility, projection switching, projection visibility summary, proof-family catalog inspection, prompt/inference audit artifact inspection, static/auth/settlement family proof inspection, and internal-versus-reviewer source-material visibility | V17 should keep growing beyond those flows into broader operator and failure-path behavior | browser suite asserts surfaced proof/artifact/operator states beyond shell order alone and can now exercise principal-bounded visibility, raw proof-family inspection, member/theorem surfaces, and source-material visibility boundaries directly in the demo shell | strongly advanced |
...
| Proof/demo alignment | V17 now has integration and E2E ratchets for proof-contract replay visibility, bounded-public disclosure, surfaced proof/disclosure panels, proof-family catalog rendering, restrictive raw proof/materialization inspection, prompt/inference audit artifacts, static/auth/settlement family proofs, and reviewer/public replay-artifact boundaries | V17 should continue extending those ratchets across the remaining V16 audit seams | test failures reveal proof/demo drift rather than only static artifact mismatches; this pass found and fixed the stale `ENGI_NEED.md` markdown body before it could masquerade as proof-complete operator documentation | strongly advanced |
...
1. The V16 spec/parity materials are now materially exercised, the full seeded scenario corpus is covered through HTTP workflows across both branch modes with exact family/member/theorem catalog checks, and the highest-risk family-member semantics now have dedicated workflow ratchets, but the audit is still not exhaustive across every family/member/scenario/principal permutation at the same operator-realism depth.
...
4. Projection-difference coverage is materially stronger, and the direct proof-artifact surfaces now exist in the demo, but not every proof/artifact surface has been exercised across every principal and every scenario family.
5. Replay-surface closure is now ratcheted generically in unit tests, exactly across the seeded corpus, and semantically across the highest-risk families, but more family/member-specific workflow demonstrations are still available.
