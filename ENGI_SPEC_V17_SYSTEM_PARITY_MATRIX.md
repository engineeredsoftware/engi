...
1. The V16 spec/parity materials are now materially exercised, and the full seeded scenario corpus is covered through HTTP workflows across both branch modes with exact family/member/theorem catalog checks, but the audit is still not exhaustive across every family/member/scenario/principal permutation at the same operator-realism depth.
