...
| Workflow integration coverage | `workflow.integration.test.js` now exists and carries closer-to-real ENGI workflow paths alongside the retained API boundary tests | V17 should keep growing workflow integration as a first-class suite rather than letting it collapse back into generic route checks | dedicated workflow integration entrypoint exists, is reflected in scripts/coverage metadata, and now includes internal/reviewer/public workflow differences plus restrictive verification/materialization behavior | strongly advanced |
| Endpoint validation coverage | `api.test.js` covers malformed input, static serving, unknown routes, reset behavior, and persistence failures | V17 should preserve endpoint validation while distinguishing it from workflow integration | endpoint-only checks stay present, but are no longer the only non-unit non-e2e middle layer | partially aligned |
...
| Browser E2E breadth | browser tests now cover targeted settlement, normalization/source-to-shares, privacy-boundary identity/auth plus proof/disclosure surfaces, restrictive raw proof inspection, and projection switching | V17 should keep growing beyond those flows into broader operator and failure-path behavior | browser suite asserts surfaced proof/artifact/operator states beyond shell order alone and can now exercise principal-bounded visibility directly in the demo shell | strongly advanced |
| Demo/operator truth | `public/app.js` and `server.js` already expose a coherent V16 operator shell | V17 should use that shell as the production-like demonstration surface rather than a thin demo-only afterthought | UI-facing tests and runtime docs treat the demo as the primary demonstration vehicle | required posture |
| Projection-difference coverage | source already supports `public`, `buyer`, `reviewer`, and `internal` projections, and V17 now exercises them across unit, API, integration, and browser layers | V17 should ratchet projection differences directly across unit, API, integration, and demo surfaces | tests verify raw-file visibility, replay-surface visibility, bounded-public omission, and browser-visible projection switching across principals | strongly advanced |
...
| Browser failure/reset coverage | browser coverage initially focused on happy-path flows | V17 should surface operator-visible validation and reset behavior directly in the interface suite | E2E tests cover invalid deposit feedback and reset-to-seeded-state after a realized run | substantially advanced |
| Proof/demo alignment | V17 now has integration and E2E ratchets for proof-contract replay visibility, bounded-public disclosure, surfaced proof/disclosure panels, and raw restrictive proof/materialization inspection | V17 should continue extending those ratchets across the remaining V16 audit seams | test failures reveal proof/demo drift rather than only static artifact mismatches | strongly advanced |
...
The concrete opening debts currently in view are:

1. The V16 spec/parity materials are now materially exercised, but they are not yet fully exhausted as a V17 audit checklist.
2. Endpoint validation and workflow integration are better separated, but `api.test.js` still carries some workflow-adjacent coverage.
3. Browser coverage is now meaningfully broader, but it still does not cover every high-risk operator workflow or failure mode.
...
6. The demo-driven bug-finding loop is now materially stronger, but it should keep expanding from these higher-signal seams into fuller operator realism.
