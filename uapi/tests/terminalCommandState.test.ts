import { normalizeTerminalCommandState } from '@/app/terminal/terminal-command-state';

describe('normalizeTerminalCommandState', () => {
  it('derives command options and posture from the shell snapshot', () => {
    const state = normalizeTerminalCommandState({
      selection: {
      ...
      commandSurface: {
        heroLede: 'Active canon Terminal posture',
  ...
  it('falls back safely when command-surface data is sparse', () => {
    ...
    expect(state?.heroTip).toBe('The current flow guidance and witness signals are loading.');
