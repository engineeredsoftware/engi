import {
  ...
  buildApplicationReadinessSummary,
  normalizeApplicationLiveSummary,
} from '@/app/application/application-live-summary';
...
describe('normalizeApplicationLiveSummary', () => {
  ...
  it('builds explicit Terminal readiness items for reconnect-required repository and settlement posture', () => {
    const items = buildApplicationReadinessSummary({
  ...
  it('merges readiness items ahead of the shell summary bridge without duplicating labels', () => {
    const items = buildApplicationLiveSummary(
      {
        summarySurface: [
