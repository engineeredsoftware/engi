...
describe('searchRelevantAssetPackEvidence', () => {
  ...
  it('uses count override for post-context', async () => {
    ...
    await searchRelevantAssetPackEvidence({ repoOwner: 'o', repoName: 'r', repoBranch: 'b', repoCommit: 'c', stage: 'post-context', count: 2 });
  ...
  it('returns empty on RPC error', async () => {
    ...
    const res = await searchRelevantAssetPackEvidence({ repoOwner: 'x', repoName: 'y', repoBranch: 'z', repoCommit: 'w', stage: 'pre-context' });
    expect(res).toEqual([]);
  });
});
