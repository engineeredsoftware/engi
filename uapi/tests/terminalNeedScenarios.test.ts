...
} from '@/app/terminal/terminal-need-scenarios';

describe('normalizeTerminalNeedScenarios', () => {
  it('normalizes active scenario cards and need posture from the shell snapshot', () => {
    const state = normalizeTerminalNeedScenarios({
      selection: {
  ...
  it('returns null for empty snapshots', () => {
    expect(normalizeTerminalNeedScenarios(null)).toBeNull();
  });

  it('normalizes Exchange Need-fitting review state for Terminal review controls', () => {
    const review = normalizeTerminalNeedFittingReview({
      scenario: {
