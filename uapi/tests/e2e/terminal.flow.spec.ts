...
test.describe('Bitcode Terminal browser flow', () => {
  test('Terminal route keeps read, selection, and repository-anchor write-through in one Terminal surface', async ({
    page,
    context,
  }) => {
    ...
    await page.goto('/terminal?provider=github&repo=bitcode%2Fterminal&transactionId=run-proof&transactionDetail=activity');
