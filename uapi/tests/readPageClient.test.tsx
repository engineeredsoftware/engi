...
jest.mock('@/app/terminal/TerminalRepositoryContextPanel', () => ({
  __esModule: true,
  default: ({ onContextChange }: { onContextChange: (value: unknown) => void }) => {
    React.useEffect(() => {
      onContextChange({
        provider: 'github',
        selectedRepository: {
    ...
    return <section aria-label="Repository source selector">Repository source selector</section>;
  },
}));

jest.mock('@/app/terminal/TerminalReadScenarioPanel', () => ({
  __esModule: true,
  default: () => <section aria-label="Read request scenarios">Read request scenarios</section>,
}));
...
describe('ReadPageClient', () => {
  ...
  it('renders the five-step /read route with source-safe session state and live workbench ownership', async () => {
    ...
    expect(screen.getByTestId('read-route-step-request-read')).toBeInTheDocument();
    expect(screen.getByTestId('read-route-step-review-synthesized-need')).toBeInTheDocument();
    expect(screen.getByTestId('read-route-step-request-fit')).toHaveAttribute('data-reading-step-state', 'current');
    expect(screen.getByText('Source-safe read state')).toBeInTheDocument();
    expect(screen.getByText('ReadNeedComprehensionSynthesis')).toBeInTheDocument();
    expect(screen.getByText('ReadFitsFindingSynthesis')).toBeInTheDocument();

    await waitFor(() => expect(screen.getByLabelText('Repository source selector')).toBeInTheDocument());
    const workbench = screen.getByLabelText('Reading workbench');
    await waitFor(() => expect(workbench).toHaveAttribute('data-admitted-read', 'read-admission-1'));
