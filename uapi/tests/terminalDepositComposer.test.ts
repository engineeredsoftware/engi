import { normalizeTerminalDepositComposer } from '@/app/terminal/terminal-deposit-composer';

describe('normalizeTerminalDepositComposer', () => {
  it('normalizes selected inventory and auth-session defaults from the shell snapshot', () => {
    const state = normalizeTerminalDepositComposer({
      selection: {
  ...
  it('returns null for empty snapshots', () => {
    expect(normalizeTerminalDepositComposer(null)).toBeNull();
  });
});
