...
describe('PacksPageClient', () => {
    ...
    global.fetch = jest.fn(async () => ({
      ...
      json: async () => ({
        ...
        records: [
          {
            ...
            measurements: [{ id: 'measured-btd', label: 'Measured btd', value: 42, unit: 'BTD', root: null }],
            values: [{ id: 'btc-fee', label: 'Btc fee', amount: 3200, unit: 'sats' }],
            proofRoots: [{ id: 'settlement-root', label: 'Settlement root', root: 'settlement-root-def' }],
            sourceSafety: {
        ...
        detail: {
          ...
          telemetry: { sourceEventId: 'pack-activity-1', sourceKind: 'execution', sourceChannel: 'system-surface' },
          metadata: {},
        },
        summary: {
          total: 1,
  ...
  it('renders searchable Packs activity and source-safe detail readback', async () => {
    ...
    expect(screen.getByRole('heading', { name: 'Pack activity' })).toBeInTheDocument();
    await waitFor(() => expect(screen.getAllByText('Auth rollback proof pack').length).toBeGreaterThan(0));
    ...
    expect(JSON.stringify(screen.queryByText(/protected source/i))).not.toContain('protected source body');
  });

  it('writes route query params for filters and selected detail', async () => {
    render(<PacksPageClient />);

    await waitFor(() => expect(within(screen.getByRole('table')).getByText('Auth rollback proof pack')).toBeInTheDocument());
    fireEvent.click(within(screen.getByRole('table')).getByText('Auth rollback proof pack'));
    expect(mockReplace).toHaveBeenCalledWith('/packs?q=rollback&type=read-need-fit-preview&detailId=pack-activity-1', { scroll: false });

    fireEvent.change(screen.getByLabelText('Activity type'), { target: { value: 'settlement' } });
    expect(mockReplace).toHaveBeenCalledWith('/packs?q=rollback&type=settlement', { scroll: false });
  });
});
