import { deriveApplicationTransactionReadiness } from '@/app/application/application-transaction-readiness-source';

describe('deriveApplicationTransactionReadiness', () => {
  it('prefers route-local repository truth over stale auxillary connection fallback', () => {
    const result = deriveApplicationTransactionReadiness({
      signedIn: true,
      repositoryContext: {
        connectionStatus: {
          connected: true,
