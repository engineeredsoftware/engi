...
export type ApplicationExternalRuntimeInterface = {
  interfaceId: string;
  label: string;
  runtimeState: string;
  resultClass: string;
...
export type ApplicationExternalRuntimeSnapshot = {
  configuredEnvironmentMode: string;
  actualityDisposition: string;
  interfaces: ApplicationExternalRuntimeInterface[];
  counts: {
    total: number;
...
export function normalizeExternalRuntimePayload(payload: unknown): ApplicationExternalRuntimeSnapshot | null {
  ...
  const interfaces = rawStates
    .map((entry) => {
      ...
      } satisfies ApplicationExternalRuntimeInterface;
    })
    .filter((entry): entry is ApplicationExternalRuntimeInterface => Boolean(entry));
