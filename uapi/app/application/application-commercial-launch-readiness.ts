export const SEVENTH_GATE_COMMERCIAL_TESTNET_LAUNCH_MAP = [
  {
  ...
  },
  {
    ...
    sourceBasis: [
      ...
      'uapi/tests/e2e/application.terminal.flow.spec.ts',
    ],
  },
  {
  ...
  {
    id: 'old-world-compatibility-explanations-retired',
    label: 'Old-world compatibility explanations retired',
    launchAxis: 'reform-completion',
    requiredPosture:
      'Core user journeys explain Bitcode Need, source-to-shares, AssetPack, Shippable, BTC fee, and $BTD read-right behavior directly without old-world compatibility fallback.',
    sourceBasis: [
...
export function getSeventhGateCommercialTestnetLaunchSurface(
  id: (typeof SEVENTH_GATE_COMMERCIAL_TESTNET_LAUNCH_MAP)[number]['id'],
) {
  return SEVENTH_GATE_COMMERCIAL_TESTNET_LAUNCH_MAP.find((surface) => surface.id === id) || null;
}
