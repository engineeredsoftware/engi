...
import {
  ...
  getTerminalAction,
  getTerminalExperience,
} from './terminal-experience-architecture';
          ...
          <div id={panel.id} key={panel.id} className={`rounded-[1.75rem] border px-5 py-5 ${toneForPanel(panel.id)}`}>
            <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
              ...
              <div className="flex shrink-0 flex-wrap items-center gap-2 text-[0.66rem] uppercase tracking-[0.18em] text-neutral-300">
                {(() => {
                  const experience = getTerminalExperience(CORE_PANEL_EXPERIENCE[panel.id]);
                  return experience ? (
                ...
                {(() => {
                  ...
                  const action = actionId ? getTerminalAction(actionId) : null;
                  return action ? (
