...
import {
  ...
  type TerminalExternalRuntimeSnapshot,
} from './terminal-external-runtime';
...
function buildExternalRealizationRequestPath(environmentMode: TerminalEnvironmentMode | null) {
  if (!environmentMode) {
...
interface TerminalExternalInterfacingPanelProps {
  environmentMode?: TerminalEnvironmentMode | null;
  onRecordActivity?: (draft: TerminalActivityRecordDraft) => Promise<unknown>;
}

export default function TerminalExternalInterfacingPanel({
  environmentMode = null,
  onRecordActivity,
}: TerminalExternalInterfacingPanelProps) {
  const [snapshot, setSnapshot] = useState<TerminalExternalRuntimeSnapshot | null>(null);
  ...
  const handleRecordBoundaryReadiness = async () => {
    ...
    try {
      await onRecordActivity(buildTerminalExternalInterfacingDraft(snapshot));
              ...
              <div className="mt-4 rounded-[1.3rem] border border-white/8 bg-white/5 px-4 py-4 text-sm leading-6 text-neutral-300">
                {counts?.blocking ? (
                  ...
                  <span className="inline-flex items-center gap-2 text-emerald-200">
                    ...
                    No terminal-visible interface is currently misconfigured.
                  </span>
                )}
              </div>
