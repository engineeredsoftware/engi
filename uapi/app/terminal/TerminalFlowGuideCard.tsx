...
import { deriveTerminalFlowGuide } from './terminal-flow-guide';
import type { BitcodeTransactionReadiness } from './bitcode-transaction-readiness';

interface TerminalFlowGuideCardProps {
...
export default function TerminalFlowGuideCard({
  commandState,
  continuationTip,
  transactionReadiness,
}: TerminalFlowGuideCardProps) {
  const guide = deriveTerminalFlowGuide(commandState, transactionReadiness);

  return (
    <div className="rounded-[1.5rem] border border-white/8 bg-black/20 px-5 py-5">
      <div className="flex items-center gap-2">
        <p className="text-[0.68rem] uppercase tracking-[0.24em] text-neutral-400">Deposit + Read flow guide</p>
        <span
