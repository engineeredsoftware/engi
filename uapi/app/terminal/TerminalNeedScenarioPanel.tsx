...
import {
  normalizeTerminalNeedFittingReview,
  normalizeTerminalNeedScenarios,
  type TerminalNeedFittingReviewState,
  type TerminalNeedScenariosState,
} from './terminal-need-scenarios';
...
export default function TerminalNeedScenarioPanel({ onRecordActivity }: TerminalNeedScenarioPanelProps) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/need-review?scenarioId=${encodeURIComponent(needState.selectedScenarioId)}`)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error(await readTerminalRouteError(response, 'Unable to read the active Need-fitting review.'));
        }
        return response.json();
      })
      .then((payload) => {
        if (!cancelled) {
          setNeedFittingReview(normalizeTerminalNeedFittingReview(payload));
        }
      })
      .catch((error) => {
        if (!cancelled) {
  ...
  const handleRecordActiveNeed = async () => {
    ...
    try {
      await onRecordActivity(buildTerminalNeedMeasurementDraft(needState));
  ...
  const handleReviewNeed = async (action: 'accept' | 'reject' | 'remeasure-with-feedback') => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(await readTerminalRouteError(response, 'Unable to review the active Need.'));
      }
  ...
  return (
    ...
    >
      ...
      <div className="mt-6 flex flex-wrap gap-3">
        ...
        <button
          type="button"
          onClick={() => jumpToShellSection('terminalGiveNeedWorkbench')}
