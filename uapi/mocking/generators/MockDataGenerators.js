...
exports.mockDataGenerator = exports.MockDataGeneratorEngine = void 0;
/**
 * Master mock data generator with intelligent scenario awareness
 */
class MockDataGeneratorEngine {
    constructor() {
     ...
     */
    async generate(feature, context, config) {
     ...
     */
    async generateRelated(primaryFeature, relatedFeatures, context, config) {
        const results = {};
    ...
    getCacheKey(feature, context, config) {
        return `${feature}:${context.scenario}:${context.complexity}:${JSON.stringify(config)}`;
    }
    wrapData(data, feature, context, generationTimeMs) {
    ...
    createRelatedContext(baseContext, primaryFeature, relatedFeature, primaryData) {
...
exports.MockDataGeneratorEngine = MockDataGeneratorEngine;
class BaseDataGenerator {
    getItemCount(context, config) {
    ...
    generateRealisticTimestamp(offsetDays = 0) {
        const date = new Date();
...
class UserProfileGenerator extends BaseDataGenerator {
    ...
    generateTeamMembers(context, config) {
...
class TemplateGenerator extends BaseDataGenerator {
    async generate(context, config) {
        const generateTemplateList = (category, count) => {
            const templates = [];
            for (let i = 0; i < count; i++) {
                templates.push({
...
exports.mockDataGenerator = new MockDataGeneratorEngine();
