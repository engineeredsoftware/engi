...
5. creating an inspectable GitHub remediation branch that contains the selected source material references, manifests, measurement context, and settlement preview,
...
> Measure failing benchmark slices in a buyer repo, match the right technical supply, create an inspectable remediation branch containing the relevant source material and accounting context, and produce exact attribution and settlement around the resulting value movement.
...
`repoProvider` is intentionally omitted because, in v1, repository source is already implied by `sourceProvider = 'github'`. The earlier duplication is removed.
...
## 15.6 Measurement detail: static vs inferred
...
### `hardReject` semantics

`hardReject` is **not** duplicative of the primitive fields. It is a derived control bit used by the decision layer.

Primitive fields answer:
- what happened?

`hardReject` answers:
- does this state force rejection without further consideration?

### Hard rejection rule

If any of these are false, `hardReject = true`:
- signer address exists
- signature verifies
- signed payload hash matches content root

## Review annotation
- `hardReject` is now explicitly documented as a derived control state rather than redundant storage.
...
`hardReject = true` only if provenance claims are contradicted or structured evidence strongly suggests falsity.

Missing provenance may weaken branch eligibility without causing hard reject.
...
type VerificationSufficiency = {
  ...
  recommendedUseTier: 'reject' | 'context-only' | 'patch-eligible'
  reasons: string[]
}
```
...
```ts
function decideVerificationUseTier(score: number): 'reject' | 'context-only' | 'patch-eligible' {
...
```ts
if (status === 'revoked') hardReject = true
```
...
function decideCandidateUseTier(v: CandidateVerification): CandidateUseTier {
  if (v.issuanceVerification.hardReject) return 'reject'
  if (v.provenanceVerification.hardReject) return 'reject'
  if (v.issuerPolicyStatus.hardReject) return 'reject'

  const verificationTier = v.verificationSufficiency.recommendedUseTier

  if (verificationTier === 'reject') return 'rank-only'
...
function upgradeToSettlementEligible(
  ...
  const issuanceStrong = !v.issuanceVerification.hardReject
  const provenanceStrong = !v.provenanceVerification.hardReject
  const issuerAllowed = v.issuerPolicyStatus.status === 'allowed'
...
7. ENGI writes source-material references, manifests, and settlement preview into that branch,
...
### Context branch
Contains manifests, selected source material references, and reasoning.

### Patch branch
Contains manifests plus generated remediation changes.
...
Every credited asset in the journal must exist in the locked `AssetPack`, and every referenced receipt must exist in the event bundle.
...
# 25. Utility observations

Utility observations MUST remain separate from settlement unless and until they trigger actual settlement under a later policy.

Recommended split:
- `utility_observation`
- optional future `utility_settlement`

ENGI v1 SHOULD NOT automatically equate observed benchmark lift with immediate economic movement.

## Review annotation
- This preserves conceptual clarity between evidence and settlement.
...
function checkProvenanceVerification(asset, githubCtx) {
  ...
  const hardReject = provenanceClaimContradictionExists(asset, githubCtx)
...
function checkIssuerPolicyStatus(asset, policyState) {
  ...
  const hardReject = status === 'revoked'
