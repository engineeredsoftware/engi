...
- the `packages/pipelines/asset-pack/*` corridor is the live AssetPack package owner and must mirror compatibility `definitionOfDone` / `deliverableType` state into semantic `need` / `writtenAssetType` / asset-pack snapshots at execution-store and postprocess boundaries so later-gate reform can remove old naming without losing protocol meaning;
...
- client-side execution hooks that call the `/api/deliverables` endpoint must describe the live behavior as Bitcode asset-pack pipeline submission, keep `definitionOfDone` only as compatibility input naming, and keep tracked runtime mirrors aligned with the TypeScript source;
