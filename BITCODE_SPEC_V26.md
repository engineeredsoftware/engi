...
- retained raw promptparts and promptpart-generation scripts for this corridor should likewise describe discovery/implementation/validation/shipping in asset-pack-run, need-satisfaction, written-asset, and shipping-wrapper terms, even when compatibility filenames and identifiers still include `deliverable`; this includes deeper phase-purpose, setup-comprehension, shipping-finalization, deliverables-system, implementation-divider, create-code-change, PR-packaging, and create-pull-request promptparts, which must no longer teach PR-first or GA-1-first semantics where Bitcode now requires need-first asset-pack synthesis plus shipping wrappers;
