...
- the retained `packages/pipelines/deliverable/*` corridor must also resolve live phase, validation, shipping, and reread behavior from semantic `writtenAssetType` / `need` first, with `deliverableType` and `definitionOfDone` remaining only compatibility carriers, because V26 is shaping the protocol together with the commercial infrastructure into the first full-repository-as-proven-products Bitcode canon rather than leaving commercial surfaces as semantically secondary wrappers;
