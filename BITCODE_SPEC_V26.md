...
- admitted `Bitcode MCP` workflow/development prompts and the retained `bitcode://pipelines/deliverable/create` tool description must teach the live operation as a Bitcode asset-pack pipeline over source-to-shares needs, with retained compatibility URI/subtype names called out explicitly; package-local tests and generated source-content proof must render these prompt/tool surfaces and reject `deliverable pipeline` as active MCP canon;
