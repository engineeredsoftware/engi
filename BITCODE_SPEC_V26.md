...
- AssetPack Finish/Delivering filesystem ownership must live under `agents/finish/*`, `agents/finish-compatibility/*`, and `finish-delivery-compatibility-agents.ts`; any `shipping:*` registry keys are compatibility aliases and must not be taught as broad phase ownership;
