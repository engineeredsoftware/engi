...
- wallet identity in `Profile` plus GitHub or equivalent repository scope in `Connects` are enforced as the minimum transactional readiness posture, and settlement-bearing route admission must prove the selected repository anchor belongs to the connected provider inventory before deposit or branch materialization can proceed,
...
- the primary auxillary reread carrier at `/api/auxillaries/data` must reuse that same stored-first/live-fallback repository inventory contract, expose `repositoryInventorySource`, and feed `Connects` with the same repository-scope truth enforced by deposit and branch admission,
