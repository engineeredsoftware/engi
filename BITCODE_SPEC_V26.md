...
| Repository anchor and VCS scope | repository anchoring is explicit, Bitcode-owned, and admitted through `Connects` / `/application` / API surfaces | repository context no longer depends on stale provider-caller drift; Bitcode write flows can bind repository scope coherently | `packages/vcs/*`, `packages/github/*`, `packages/api/src/vcs/*`, `uapi/app/api/vcs/*`, repository context application panels | repository-boundary compile health plus repository-anchor behavior proof |
