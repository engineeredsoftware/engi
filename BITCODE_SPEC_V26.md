...
| Need review before fit search | synthesized Needs are presented for review after measurement and before fitting, with accept, reject, and remeasure-with-feedback outcomes | `.bitcode/need-review.json` records the reviewable Need, applied decision, source-to-shares focus, and fit-search admission state; candidate recall/ranking starts only after accepted review | `protocol-demonstration/src/canonical/need-measurement.js`, `protocol-demonstration/src/bitcode-demo.js`, `protocol-demonstration/src/canonical/evaluation-materialization.js` | `protocol-demonstration/test/v26-need-review-source-to-shares.test.js`, branch-artifact required-path checks, and proof-witness digest coverage |
...
| Settlement and follow-through | settlement posture, branch artifacts, proof state, quantized source-to-shares fit qualities, and closure metrics remain in the same Bitcode-owned reread model | closure-bearing writes persist meaningful follow-through instead of thin execution notes; selected detail can reconstruct saved settlement state and receipts carry the same quantized fit-quality objective shown at present-fit-for-settlement-review time | `application-activity-history`, `application-transaction-detail-snapshot`, closure/detail cards, retained execution-history rows, `protocol-demonstration/src/canonical/settlement.js` | closure-follow-through persistence proof, reread tests, and `v26-need-review-source-to-shares` receipt/proof coverage |
