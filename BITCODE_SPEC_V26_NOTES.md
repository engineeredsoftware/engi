...
- `make shares` now includes a mandatory post-measurement pre-fit Need review: `.bitcode/need-review.json` must expose a reviewable Need with accept, reject, and remeasure-with-feedback outcomes, and fit search is admitted only after the Need is accepted for source-to-shares settlement review
...
- `/application` is not fifth-gate-complete while its command, deposit, closure, or detail controls only poke preserved shell controls; those write surfaces must resolve into protocol-owned routes and then reread through the same Bitcode activity ledger
- protocol-owned branch/deposit writes and retained execution-history rows are both active fifth-gate truth now; if one can write while the Bitcode Terminal cannot immediately reread and select the resulting activity, the system is still split
...
- projected live rows are still insufficient if they only carry list-level posture; the selected-detail fallback for `protocol-projection` rows must preserve the same repository/give/need/fit/supply structure as the projected ledger card, otherwise live protocol posture still collapses when the operator opens detail before retained persistence catches up
- closure-bearing writes are still split if they persist only a thin execution note while settlement/proof/history follow-through remains shell-only; fifth-gate now expects persisted rows to carry enough closure follow-through for the Bitcode Terminal detail surface to reread settlement posture even after runtime shell drift
- the same rule now extends to richer closure panels too: persisted Bitcode activity rows must carry enough verification/branch/settlement/ledger structure for selected detail to remain readable even when live shell state is absent, otherwise `/application` is still teaching two state models
- the same persistence rule now extends to non-closure share/use posture too: persisted Bitcode activity rows must carry enough repository-anchor, give, need, fit, and supply-selection structure for selected detail and the activity tab to reread saved posture when live execution streaming is empty, otherwise Bitcode Terminal is still splitting shell truth from saved ledger truth
