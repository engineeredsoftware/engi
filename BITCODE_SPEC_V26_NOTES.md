...
- `uapi/tests/applicationReadScenarios.test.ts` now proves deterministic normalization of active scenario cards, parser posture, closure counts, and target-kind counts into route-local read-scenario state.
...
- `uapi/tests/applicationDepositReadWorkbench.test.ts` now proves deterministic deposit/read/fit normalization from the semantic shell snapshot into application-owned action detail.
...
- duplicate JavaScript mirrors are now removed from `uapi/app` across App Router entrypoints, route-local components, hooks, helper modules, and API shared carriers, and `protocol-demonstration/test/v26-uapi-app-router-entrypoints.test.js` proves the active Bitcode app-owned boundary stays TypeScript-only.
