...
- the same non-mock conversation execution seam now persists normalized `rich_input` evidence on the execution row too: source attachments, output destinations, asset-pack references, and need-measurement intents are separated from raw token arrays so Exchange rereads and Terminal proof views can prove what source-to-shares write intent actually entered the run
...
- the Bitcode Terminal closure map and native need-scenario controls now also have to show that review boundary directly: reviewable Need admission appears before verification and settlement, operators can accept/reject/remeasure with feedback before fitting, `ApplicationNeedScenarioPanel` must fetch and normalize the Exchange `needFittingReview` payload, and selected-detail persistence must carry that review panel forward instead of collapsing it into a raw branch artifact
...
- retained Jira/use-computer caller drift counts the same way too: `packages/generic-tools/{mcps-tools/jira/src/index.ts,multimodal-processing/src/index.ts,use-computer/src/index.ts}` plus `packages/pipelines/asset-pack/src/tools/BitcodeNeedMeasurementComputerUseTool.ts` must not preserve nullable Jira-connection assumptions, stale multimodal class exports after a factory-tool refactor, or the older schema-class `Tool` pattern for shell-execution tools; those are caller-layer carry-over errors above the reusable Bitcode tool primitive boundary
