...
- the same admissibility boundary now explicitly includes MCP interface precision too: `packages/executions-mcp/{README.md,src/mcp-server/src/{types/index.ts,pipeline-execution/adapter.ts}}` must distinguish primitive `@bitcode/mcp` utilities from the Bitcode Exchange-facing MCP server, treat repository/provider connections and attachments as execution inputs, and treat retained deliverable-style outputs as Bitcode asset packs rather than leaving MCP ingress/egress meaning implicit
