...
| `DeliverableType.ts` package primitive | `AssetPackWrittenAssetType.ts` and `AssetPackWrittenAssetType` | package-owned type/export names now teach written-asset kinds directly while route payloads may still carry compatibility `deliverableType` fields as mirrors |
