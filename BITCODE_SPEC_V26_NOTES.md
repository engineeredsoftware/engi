...
- active checkout and BTD-notification execution paths are inside that same boundary too: `uapi/app/{api/create-checkout-session,api/fulfill-checkout-session,api/notifications/{low-btd-reminder,out-of-btd,btd-transfer},checkout/callback/{page,CheckoutCallbackClient}}`, `uapi/networking/api-client.ts`, and `supabase/templates/{low_btd_reminder,out_of_btd,btd_transfer}.html` must be canonical owners rather than routing through `app/_legacy`, and they must keep `$BTD` purchase/balance/transfer wording plus BTD-named template and fetch paths rather than preserving `credit-transfer`, `low-credits-reminder`, `out-of-credits`, or `_legacy/tps/stripe/checkout`.
