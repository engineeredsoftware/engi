...
- `uapi/components/base/engi/layout/workspace-surface.ts`, `ClientLayoutInner.tsx`, `nav.tsx`, and `workspaceSurface.test.ts` now centralize workspace-route classification so `/application`, `/orbitals`, and `/conversations` share one operator-chrome rule for navbar surface posture and footer suppression.
