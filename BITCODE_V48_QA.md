...
- **F27 (next — same envelope bug, deposit-critical)**: the DEPOSIT agents (`deposit-asset-pack-synthesis-agent` reads `result.options`, `deposit-input-comprehension` reads `result.comprehension`, `deposit-depository-search` reads `result.guidance`) read the PTRR envelope directly too — so their real outputs were being dropped to the fallback defaults (likely why deposit synthesis "yielded no admissible options"). Apply the same `finalOutput ?? output` unwrap.
