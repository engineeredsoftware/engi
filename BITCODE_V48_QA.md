...
- Remaining (throughput — decision pending): make the interactive deposit fast enough for the 300s route. Options: (a) make the deposit agents respect the bounded profile (bounded → one bounded structured-inference call per agent, like the read agents; full → the rich PTRR) — matches the `PROFILE=bounded` the operator set; (b) background the synthesis (decouple from the request lifetime so the full rich pipeline can run while telemetry streams). Tradeoff: telemetry richness vs interactive speed.
