...
| Reading | strong measured-read surfaces | read becomes the first-class demand surface in the shell | P0 |
| Deposit-to-read fit | mostly inferable through existing surfaces | explicit fit surface shown before deep artifacts | P0 |
| Profile meaning | targeted vs normalization distinction is present | profile distinction becomes obvious from deposit/read/closure shape | P0 |
...
- `src/bitcode-demo.js` emits `depositingSurface`, `readingSurface`, and `depositingToReadingSurface` on `latestRun`
...
- `publicState()` now exposes a `readingSurface` per seeded scenario so the shell can foreground read before a run
...
## Phase 2 — reading foregrounding

### Goal
Make reading feel like measured demand rather than background parser metadata.

### Required changes
- foreground task/failure/closure summaries in the shell
- make boundedness/compositeness obvious in the active profile
- render target artifact kinds and closure criteria as read-facing summaries
...
## Phase 3 — deposit-to-read fit surface

### Goal
Explicitly show why the current deposit matches the current read.

### Required changes
- add a deposit-to-read fit surface to `src/bitcode-demo.js`
...
- tie them explicitly to deposit mode, read mode, asset-pack shape, and settlement shape
...
- adjust the shell and ordering so they land after deposit/read/fit are already understood
...
- reading-first summary assertions
- deposit-to-read fit surface presence and coherence
- profile distinction assertions tied to deposit/read semantics
...
2. depositing and reading are visibly foregrounded,
3. deposit-to-read fit is explicit,
