...
| `packages/pipelines/asset-pack/src/*` old-world naming | `trace-and-reform` | names like `deliverable`, `deliverableType`, `finalWorkSummary`, and `comprehend-task` are search beacons for reform, not retained functionality; the live filesystem/package owner is AssetPack and V26 teaches `asset pack`, `AssetPack completion`, `written asset`, `Shippable`, and `comprehend need`; see `protocol-demonstration/V26_SHIPPABLE_REFORM.md` |
