...
| `packages/pipelines/multi/src/*` | `cut` | old-world multi-agent / multi-deliverable pipeline selection is removed from V26 rather than admitted as a live Bitcode product control |
...
- email-template filenames and promptpart identifiers in this corridor should also stay compatibility-only while their rendered copy and prompt content teach asset-pack-run, written-asset, and shipping-mechanism semantics
- raw promptparts and promptpart-generation scripts in this corridor should likewise stay compatible by identifier while their actual prompt text and generation guidance describe asset-pack-run, need-satisfaction, written-asset, and delivery-mechanism semantics; deeper phase-purpose, setup-comprehension, shipping-finalization, deliverables-system, implementation-divider, create-code-change, PR-packaging, create-pull-request, ready-to-ship, code-change-readiness, and code-change-review-readiness promptparts should therefore teach Bitcode-first execution meaning instead of PR-first / deployment-ceremony-first / V26 residue
- `packages/pipelines/deliverable/*` setup and prompt carriers should prefer canonical `comprehend-need` module paths and `DELIVERABLESETUPCOMPREHENDNEED` PromptParts, and discovery outputs should mirror compatibility `deliverables` / `definitionOfDone` fields into semantic `writtenAssets` / `needSatisfactionCriteria` so the commercial infrastructure expresses Bitcode meaning before later-gate renames
- `packages/pipelines/deliverable/*` must also typecheck locally through the MCP/VCS/prompt/search support graph it still traverses; fifth-gate no longer treats runtime-only proof as sufficient for this corridor once it is claimed as live commercial infrastructure
