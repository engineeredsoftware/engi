...
- generic prompt-bearing tools follow the same rule before reuse: `packages/generic-tools/need-comprehension` is now the canonical package boundary for the former task-comprehension corridor, and its DocCode prompts, raw promptparts, README, and primitive placeholders define Bitcode need comprehension with written-asset, asset-pack, proof, and delivery-mechanism semantics rather than carrying old task-first or experimental prompt language into the fifth-gate substrate
