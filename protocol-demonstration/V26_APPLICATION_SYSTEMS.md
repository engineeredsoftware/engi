...
- generic prompt-bearing tools follow the same rule before reuse: `packages/generic-tools/task-comprehension` is still task-named for compatibility, but its DocCode prompts, raw promptparts, README, and primitive placeholders now define Bitcode need comprehension with written-asset, asset-pack, proof, and shipping-wrapper semantics rather than carrying old task-first or experimental prompt language into the fifth-gate substrate
