...
- proof surface:
  ...
  fifth-gate proof now also covers the first dependent package hop beyond that normalization boundary through `packages/generic-agents/tech-types-identifier/{src/index.ts,README.md}` and `protocol-demonstration/test/v26-active-product-naming.test.js`, so downstream agent packages keep `technologyProfile` as the canonical output envelope instead of re-fragmenting it into local stack-only schemas.
