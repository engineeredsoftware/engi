...
- proof surface:
  ...
  fifth-gate proof now also covers the canonical checkout and BTD-notification execution paths through `uapi/app/{api/create-checkout-session,api/fulfill-checkout-session,api/notifications/{low-btd-reminder,out-of-btd,btd-transfer},checkout/callback/{page,CheckoutCallbackClient}}`, `uapi/networking/api-client.ts`, and `supabase/templates/{low_btd_reminder,out_of_btd,btd_transfer}.html`, so the live purchase/balance/transfer flow no longer routes through `app/_legacy` or old `credit-transfer` / `low-credits-reminder` / `out-of-credits` naming.
