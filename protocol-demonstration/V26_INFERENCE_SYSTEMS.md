...
Retained external search agents such as `generic-agents/web-researcher` are acceptable only when their canonical prompt owners state Bitcode external-evidence research for auxiliary source context, third-party interface planning, proof-gap review, and AssetPack planning, while old web-researcher names remain compatibility carriers and never become scraping-product, proof, delivery, or live product ownership.
...
| Asset-pack synthesis compatibility | deliverable-corridor prompts, `comprehend-need` overlays, repository-evidence search prompt owners, raw PromptParts, prompt renderer | clone/VCS, repository-evidence search, written-asset file mutation, PR, review, issue/comment, template, and delivery-wrapper tools | setup, repository-evidence search, ready-to-iterate, validation, finish, final-summary agents | registry-bearing pipeline runtime, postprocess/read models, execution history | `v26-deliverable-reform`, `v26-pipeline-finish-reform`, repository-evidence search witness, text-searcher agent witness, files-maintaining prompt witness, package typecheck boundary, prompt-system proof |
...
| External-evidence research support | `web-researcher` prompt owners, `WEBRESEARCHER` compatibility raw PromptParts, generic generation/failsafe PromptParts, and public prompt subpaths | admitted web-search/content tools only for source-attributed auxiliary evidence | `bitcodeExternalEvidenceResearcher` PTRR agent with compatibility aliases | parent `AgentExecution`/pipeline execution evidence and downstream proof/read-model references | web-researcher compatibility witness, inference implementation record, prompt-system proof, raw PromptPart TS/JS carry-through |
...
| `packages/generic-agents/web-researcher/*`, `packages/generic-tools/web-search/*`, `packages/web-search/*` | retained external search corridor now specified as admitted auxiliary-input evidence support for source-attributed need context, third-party interface planning, proof-gap review, and AssetPack planning |
...
- retained external-evidence research compatibility prompts
