...
Retained grep-backed search tools such as `simple-system-text-search` are acceptable only when their canonical prompt owner states Bitcode repository-evidence search for need measurement, source-grounding, proof inspection, and AssetPack planning, while old system-text-search names remain compatibility carriers.
