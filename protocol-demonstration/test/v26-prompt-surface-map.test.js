...
test('V26 prompt surface map keeps active, support, and reference corridors explicit', () => {
  ...
  assert.match(promptSurfaceSource, /every retained compatibility substep `intent` must name the need-first written-asset \/ asset-pack \/ proof \/ Shippable delivery-mechanism role/u);
  assert.match(promptSurfaceSource, /whole retained AssetPack compatibility raw PromptPart corpus/u);
