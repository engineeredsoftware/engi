...
test('canon posture stays aligned to the active canonical pointer', () => {
  const pointerPath = fileURLToPath(new URL('../../BITCODE_SPEC.txt', import.meta.url));
  ...
  assert.equal(CURRENT_CANON_POSTURE.activeProvenAppendixPath, `BITCODE_SPEC_${ACTIVE_CANON_VERSION}_PROVEN.md`);
});
