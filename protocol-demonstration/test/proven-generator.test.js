...
test('V26 proven generator renders the active Bitcode appendix while fifth through seventh gates remain open', () => {
  ...
  assert.ok(generated.markdown.includes('.engi/conversations-continuity-proof.json'));
  assert.ok(generated.markdown.includes('.engi/runs-pipelines-totality-proof.json'));
  assert.ok(generated.markdown.includes('.engi/persistence-schema-totality-proof.json'));
  assert.ok(generated.markdown.includes('.engi/prompt-system-totality-proof.json'));
  assert.ok(generated.markdown.includes('.engi/retained-package-admissibility-proof.json'));
  ...
  assert.ok(generated.markdown.includes('.engi/v26-gate-checkpoint-report.json'));
