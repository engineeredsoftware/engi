...
/**
 * The default public state route returns `{ ok, state }`, while principal-scoped
 * routes return the projected state directly.
 *
 * @param {{ json: any }} response
 * @returns {any}
 */
function readProjectedState(response) {
...
async function withApp(t, fn) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bitcode-demo-test-'));
...
testAny('GET /api/state returns seeded active-canon public state', async (t) => {
  await withApp(t, async ({ app }) => {
    ...
    assert.equal(state.specVersion, CURRENT_CANON_POSTURE.specVersionLabel);
    ...
    assert.ok(state.repoSupplySurface.repoCount >= 1);
    assert.equal(state.boundaryRealitySurface.posture, 'honest-local-prototype');
    assert.ok(state.githubAppSessions.every((/** @type {any} */ session) => session.authPayloadHash));
    assert.ok(state.repoArtifactInventory.every((/** @type {any} */ entry) => entry.provenance?.provenanceHash));
    assert.ok(state.profileCompositions.profiles.length === 2);
...
testAny('GET /favicon.svg serves the demo icon', async (t) => {
  await withApp(t, async ({ app }) => {
    ...
    assert.match(response.text, /fill="#67FEB7"/);
  });
});
...
testAny('POST /api/deposits adds a candidate asset and ledger account', async (t) => {
  await withApp(t, async ({ app }) => {
    ...
    const stateResponse = await invoke(app, { method: 'GET', url: '/api/state' });
    const state = readProjectedState(stateResponse);
    assert.equal(state.assets.length, 12);
    assert.equal(state.assets[0].title, 'Operator playbook');
    assert.equal(state.ledger.accounts[`supplier:${response.json.asset.assetId}:pending_claims`], '0');
  });
});
...
testAny('POST /api/deposits accepts authenticated repo artifact selection without raw-only content', async (t) => {
  await withApp(t, async ({ app }) => {
    const stateResponse = await invoke(app, { method: 'GET', url: '/api/state' });
    const state = readProjectedState(stateResponse);
    const authSession = state.githubAppSessions.find((/** @type {any} */ session) => session.repo === 'frontier/demo-auth');
    const inventoryEntryIds = state.repoArtifactInventory
...
testAny('POST /api/make-bitcode-branch defaults to bounded public projection', async (t) => {
  await withApp(t, async ({ app }) => {
    const response = await invoke(app, {
      method: 'POST',
      url: '/api/make-bitcode-branch',
      headers: { 'Content-Type': 'application/json' },
      body: '{}'
    });
...
testAny('POST /api/make-bitcode-branch can run a non-default seeded scenario', async (t) => {
  await withApp(t, async ({ app }) => {
    const response = await invoke(app, {
      method: 'POST',
      url: '/api/make-bitcode-branch',
...
testAny('POST /api/make-bitcode-branch supports buyer projection and context branch mode', async (t) => {
  await withApp(t, async ({ app }) => {
    const response = await invoke(app, {
      method: 'POST',
      url: '/api/make-bitcode-branch',
...
testAny('POST /api/make-bitcode-branch accepts V23 payment mode and projects bitcoin surfaces by principal', async (t) => {
  await withApp(t, async ({ app }) => {
...
testAny('GET /api/v24/external-realization promotes enabled local executors to live-configured runtime state', async (t) => {
  await withApp(t, async ({ app }) => {
    await withEnv({
      ...
      BITCODE_V24_ENABLE_LOCAL_EXECUTORS: '1',
      BITCODE_V24_ENABLE_BITCOIN_MAINCHAIN: '1',
      BITCODE_V24_ENABLE_REPEATED_READ_PAYMENT: '1',
      BITCODE_V24_ENABLE_SIDECHAIN: '1',
      BITCODE_V24_ENABLE_COMPUTE: '1',
      ...
      assert.equal(activeRuntime.activeBindings.bitcoinMainchain.executorUrl, 'bitcode-local://bitcoin-mainchain-execution');
      assert.equal(activeRuntime.activeBindings.repeatedReadPayment.executorUrl, 'bitcode-local://repeated-read-payment-execution');
      assert.equal(activeRuntime.activeBindings.sidechain.executorUrl, 'bitcode-local://sidechain-execution');
      assert.equal(activeRuntime.activeBindings.compute.executorUrl, 'bitcode-local://compute-container-execution');
      assert.equal(activeRuntime.activeBindings.storage.executorUrl, 'bitcode-local://storage-container-execution');
      assert.equal(activeRuntime.activeBindings.github.executorUrl, 'bitcode-local://github-live-interface');
    });
  });
});
...
testAny('GET /api/v24/external-realization marks secret-dependent GitHub App execution as live-misconfigured when key material is absent', async (t) => {
  await withApp(t, async ({ app }) => {
    await withEnv({
      BITCODE_V24_ENVIRONMENT_MODE: 'development',
      BITCODE_V24_ENABLE_GITHUB: '1',
      BITCODE_V24_GITHUB_EXECUTOR_URL: 'https://github.example.test/api',
      ...
      assert.deepEqual(githubRuntime.missingSecretEnvKeys, ['BITCODE_V24_GITHUB_APP_PRIVATE_KEY_PEM']);
    });
  });
});
  ...
  await withApp(t, async ({ app }) => {
    const response = await invoke(app, {
      ...
      body: JSON.stringify({
        ...
        branchName: 'bitcode-review/demo-route',
        binding: {
          appRef: 'github-app://bitcode/development',
          appId: 'bitcode-development-github-app',
          installationTargetRef: 'github-installation://bitcode/development'
        },
        telemetry: {
...
testAny('POST /api/make-bitcode-branch realizes enabled V24 local executors across all external interfaces before persisting state', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    await withEnv({
      ...
      assert.equal(persisted.latestRun.externalEnvironmentProfile.activeBindings.github.executorUrl, 'bitcode-local://github-live-interface');
...
testAny('POST /api/make-bitcode-branch realizes the V24 repeated-read payment executor for repeated-read payment mode', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    await withEnv({
      BITCODE_V24_ENVIRONMENT_MODE: 'development',
      BITCODE_V24_ENABLE_LOCAL_EXECUTORS: '1',
...
testAny('POST /api/make-bitcode-branch persists V24 external continuity artifacts across consecutive realized runs', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    const env = {
      BITCODE_V24_ENVIRONMENT_MODE: 'development',
      BITCODE_V24_ENABLE_LOCAL_EXECUTORS: '1',
...
testAny('POST /api/make-bitcode-branch fails closed when consecutive same-mode V24 bindings drift', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    const stableEnv = {
      BITCODE_V24_ENVIRONMENT_MODE: 'development',
      BITCODE_V24_ENABLE_LOCAL_EXECUTORS: '1',
    ...
    await withEnv({
      ...stableEnv,
      BITCODE_V24_GITHUB_INSTALLATION_TARGET_REF: 'github-installation://bitcode/development-drifted'
    }, async () => {
      const response = await invoke(app, {
        method: 'POST',
        url: '/api/make-bitcode-branch',
...
testAny('POST /api/make-bitcode-branch realizes protocol-specific V24 remote adapters across all external interfaces before persisting state', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    ...
    await withEnv({
      BITCODE_V24_ENVIRONMENT_MODE: 'staging',
      BITCODE_V24_ENABLE_BITCOIN_MAINCHAIN: '1',
      BITCODE_V24_BITCOIN_MAINCHAIN_EXECUTOR_URL: `${executor.baseUrl}/bitcoin-rpc`,
      BITCODE_V24_BITCOIN_MAINCHAIN_EXECUTOR_KIND: 'bitcoin-json-rpc-v1',
      BITCODE_V24_BITCOIN_MAINCHAIN_RPC_USER: 'btc_user',
      BITCODE_V24_BITCOIN_MAINCHAIN_RPC_PASSWORD: 'btc_pass',
      BITCODE_V24_ENABLE_SIDECHAIN: '1',
      BITCODE_V24_SIDECHAIN_EXECUTOR_URL: `${executor.baseUrl}/sidechain-rpc`,
      BITCODE_V24_SIDECHAIN_EXECUTOR_KIND: 'sidechain-json-rpc-v1',
      BITCODE_V24_SIDECHAIN_RPC_USER: 'side_user',
      BITCODE_V24_SIDECHAIN_RPC_PASSWORD: 'side_pass',
      BITCODE_V24_ENABLE_COMPUTE: '1',
      BITCODE_V24_COMPUTE_EXECUTOR_URL: `${executor.baseUrl}/compute`,
      BITCODE_V24_COMPUTE_EXECUTOR_KIND: 'compute-http-v1',
      BITCODE_V24_COMPUTE_BEARER_TOKEN: 'compute_demo_token',
      BITCODE_V24_ENABLE_STORAGE: '1',
      BITCODE_V24_STORAGE_EXECUTOR_URL: `${executor.baseUrl}/storage`,
      BITCODE_V24_STORAGE_EXECUTOR_KIND: 'storage-http-v1',
      BITCODE_V24_STORAGE_BEARER_TOKEN: 'storage_demo_token',
      BITCODE_V24_ENABLE_GITHUB: '1',
      BITCODE_V24_GITHUB_EXECUTOR_URL: `${executor.baseUrl}/github`,
      BITCODE_V24_GITHUB_EXECUTOR_KIND: 'github-rest-v3',
      BITCODE_V24_GITHUB_BEARER_TOKEN: 'gh_demo_token'
    }, async () => {
      const response = await invoke(app, {
        method: 'POST',
        url: '/api/make-bitcode-branch',
...
testAny('POST /api/make-bitcode-branch realizes the V24 GitHub App adapter with installation-token exchange and receipt telemetry', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    ...
    await withEnv({
      BITCODE_V24_ENVIRONMENT_MODE: 'development',
      BITCODE_V24_ENABLE_GITHUB: '1',
      BITCODE_V24_GITHUB_EXECUTOR_URL: `${executor.baseUrl}/github-app`,
      BITCODE_V24_GITHUB_EXECUTOR_KIND: 'github-app-rest-v3',
      BITCODE_V24_GITHUB_APP_PRIVATE_KEY_PEM: githubAppPrivateKeyPem
    }, async () => {
      const response = await invoke(app, {
        method: 'POST',
        url: '/api/make-bitcode-branch',
...
testAny('POST /api/make-bitcode-branch realizes live-configured V24 GitHub and storage executors before persisting state', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
...
testAny('POST /api/make-bitcode-branch realizes live-configured V24 bitcoin, sidechain, and compute executors before persisting state', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    const executor = await startJsonExecutorServer(async ({ url, json }) => {
      if (url === '/bitcoin') {
        return {
          ...
          artifacts: {
            bitcoinNetworkExecution: {
              ...
              executionId: 'exec_live_bitcoin',
              observationId: 'obs_live_bitcoin',
              networkRef: 'tx:testnet4:bitcode:001',
              anchorRef: 'anchor:testnet4:bitcode:001'
            },
        ...
        return {
          ...
          artifacts: {
            sidechainExecutionReceipt: {
              ...
              requestId: 'req_live_sidechain',
              observationId: 'obs_live_sidechain',
              checkpointRef: 'sidechain-checkpoint:testnet:bitcode:001'
            }
          }
        };
      }
    ...
    await withEnv({
      BITCODE_V24_ENVIRONMENT_MODE: 'staging',
      BITCODE_V24_ENABLE_BITCOIN_MAINCHAIN: '1',
      BITCODE_V24_BITCOIN_MAINCHAIN_EXECUTOR_URL: `${executor.baseUrl}/bitcoin`,
      BITCODE_V24_ENABLE_SIDECHAIN: '1',
      BITCODE_V24_SIDECHAIN_EXECUTOR_URL: `${executor.baseUrl}/sidechain`,
      BITCODE_V24_ENABLE_COMPUTE: '1',
      BITCODE_V24_COMPUTE_EXECUTOR_URL: `${executor.baseUrl}/compute`
    }, async () => {
      const response = await invoke(app, {
...
testAny('POST /api/make-bitcode-branch realizes the V24 repeated-read remote adapter before persisting state', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    const executor = await startJsonExecutorServer(async ({ url, method, headers, json }) => {
      if (url === '/lightning/v1/invoices' && method === 'POST') {
        ...
        return {
          ...
          invoice: 'lnbcrt1bitcode-live-read-001',
    ...
    await withEnv({
      BITCODE_V24_ENVIRONMENT_MODE: 'staging',
      BITCODE_V24_ENABLE_REPEATED_READ_PAYMENT: '1',
      BITCODE_V24_REPEATED_READ_EXECUTOR_URL: `${executor.baseUrl}/lightning`,
      BITCODE_V24_REPEATED_READ_EXECUTOR_KIND: 'lightning-http-v1',
      BITCODE_V24_REPEATED_READ_BEARER_TOKEN: 'repeated-read-secret'
    }, async () => {
      const response = await invoke(app, {
        method: 'POST',
        url: '/api/make-bitcode-branch',
...
testAny('POST /api/make-bitcode-branch fails closed when a live-configured V24 executor errors', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
...
testAny('POST /api/make-bitcode-branch rejects unsupported principal, branch mode, payment mode, and scenario as client errors', async (t) => {
  await withApp(t, async ({ app }) => {
...
testAny('POST /api/make-bitcode-branch classifies no-survivor branch creation as a workflow conflict', async (t) => {
  await withApp(t, async ({ app }) => {
    ...
    const projectedStateResponse = await invoke(app, { method: 'GET', url: '/api/state' });
    const projectedState = readProjectedState(projectedStateResponse);
    assert.equal(projectedStateResponse.statusCode, 200);
    assert.equal(projectedState.latestRun, null);
    assert.equal(projectedState.runHistory.length, 0);
  });
});

testAny('POST /api/reset restores seeded state and clears latest run', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    await invoke(app, { method: 'POST', url: '/api/make-bitcode-branch', headers: { 'Content-Type': 'application/json' }, body: '{}' });
...
testAny('failed deposit write does not corrupt persisted demo state', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    ...
    assert.equal(state.runHistory.length, before.runHistory.length);
  });
});

testAny('failed make-bitcode-branch write does not persist settlement state', async (t) => {
  await withApp(t, async ({ app, dataPath }) => {
    const before = readPersistedState(dataPath);
    ...
    const state = readProjectedState(stateResponse);
    assert.equal(state.runHistory.length, before.runHistory.length);
    assert.deepEqual(state.ledger.accounts, before.ledger.accounts);
  });
});
