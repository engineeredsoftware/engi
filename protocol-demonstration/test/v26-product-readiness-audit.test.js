...
test('V26 product readiness audit is source-parity grounded through commercial launch and carries no stale Gate 8 blocker', () => {
  ...
  for (const product of audit.products) {
    ...
    assert.equal(product.readyForSeventhGateCommercialTestnetLaunch, true, `${product.productId} must be seventh-gate launch-ready`);
    ...
    assert.equal(product.seventhGateCommercialTestnetLaunchClaim, true, `${product.productId} must claim seventh-gate launch readiness`);
    ...
    assert.ok(product.launchEvidenceChecks.length > 0, `${product.productId} must carry launch evidence checks`);
    assert.equal(product.launchEvidencePassed, true, `${product.productId} launch evidence must pass`);
    assert.deepEqual(product.openReadiness, [], `${product.productId} must not carry stale readiness blockers`);
    assert.ok(product.closureNotes.length > 0, `${product.productId} must carry closure notes`);
  }
});
