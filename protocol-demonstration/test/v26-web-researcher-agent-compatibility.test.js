...
test('V26 web-researcher package has canonical external-evidence exports and dependencies', () => {
  ...
  assert.match(indexSource, /webResearcherAgent = bitcodeExternalEvidenceResearcher/u);
...
test('V26 web-researcher package source and runtime mirrors are residue-free', () => {
  for (const filePath of packageSourceFilePaths) {
    ...
    assert.match(source, /Bitcode external-evidence|BitcodeExternalEvidence|External evidence/u, filePath);
    assert.doesNotMatch(source, oldWorldWebResearchResiduePattern, filePath);
  }
});

test('V26 web-researcher README states auxiliary evidence-only support semantics', () => {
  assert.match(packageReadme, /Bitcode external-evidence research agent/u);
...
test('V26 web-researcher prompt files carry V26 external-evidence metadata', () => {
  for (const filePath of promptFilePaths) {
    const source = readRepoFile(filePath);

    assert.match(source, /current_version: "V26"/u, filePath);
    assert.match(source, /Bitcode external-evidence|external evidence/u, filePath);
    assert.doesNotMatch(source, oldWorldWebResearchResiduePattern, filePath);
  }
});
...
test('V26 web-researcher raw PromptParts are Bitcode-native compatibility carriers', () => {
  for (const filePath of rawPromptPartPaths) {
    ...
    assert.match(source, /current_version: "V26"/u, filePath);
    assert.match(source, /Bitcode|external evidence|auxiliary|source-attributed|AssetPack|proof|need/u, filePath);
    assert.doesNotMatch(source, oldWorldWebResearchResiduePattern, filePath);
  }
});
