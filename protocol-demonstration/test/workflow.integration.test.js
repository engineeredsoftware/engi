...
async function withApp(t, fn) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), 'bitcode-demo-workflow-test-'));
...
testAny('repo-authenticated deposit composes into a targeted branch workflow with proof and artifact closure', async (t) => {
  await withApp(t, async ({ app }) => {
    ...
    const authSession = initialState.githubAppSessions.find((/** @type {any} */ session) => session.repo === 'frontier/demo-auth');
    const inventoryEntryIds = initialState.repoArtifactInventory
...
testAny('mixed repo-backed and raw deposits compose before branch creation without losing selection or proof surfaces', async (t) => {
  await withApp(t, async ({ app }) => {
    ...
    const initialState = readProjectedState(initialStateResponse);
    const authSession = initialState.githubAppSessions.find((/** @type {any} */ session) => session.repo === 'frontier/demo-auth');
    const inventoryEntry = initialState.repoArtifactInventory.find((/** @type {any} */ entry) => entry.repo === authSession.repo);
