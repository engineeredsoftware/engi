...
| Generic retained tools | `packages/generic-tools/*` including `mcps-tools/*`, `need-comprehension`, `simple-system-text-search`, `lsp-query`, `web-search`, `vcs`, `use-computer`, `repository-setup` | retained prompt-bearing tool reservoirs and doc-code ports; `need-comprehension` is now the canonical package boundary for retained compatibility classes whose active prompts and placeholder primitives are interpreted as Bitcode need-comprehension, written-asset, asset-pack, and delivery-mechanism analysis; `simple-system-text-search` is now admitted only as bounded repository-evidence search support for need measurement and asset-pack source-grounding; `web-search` is now admitted only as source-attributed external evidence search/content retrieval for discovery-phase need synthesis |
...
The retained `packages/generic-tools/need-comprehension` family is the next generic-tool example of this rule:
