...
| AssetPack prompt ports | `packages/pipelines/asset-pack/src/{agents/prompts/*,agents/setup/*,agents/finish/*,agents/finish-compatibility/*,agents/validation-agents.ts,tools/*}` | admitted AssetPack prompt ports shaping active execution compatibility while keeping `Prompt`/`PromptPart` on public prompt subpaths and raw promptparts explicit/file-granular |
...
Their doc-comment metadata must also be Bitcode-native: `current_version` cannot preserve pre-Bitcode lineage, and every retained deliverable substep `intent` must name the need-first written-asset / asset-pack / proof / delivery-mechanism role represented by that PromptPart.
