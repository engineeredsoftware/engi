...
Its canonical prompt owners should stay local to the package usage sites under need-first names such as `AnalyzeNeedSemanticsDocCodeToolPrompt` and `ValidateNeedComprehensionDocCodeToolPrompt`, while task-named prompt files remain explicit compatibility re-export wrappers only.
