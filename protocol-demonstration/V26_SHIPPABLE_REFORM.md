...
- email-template filenames and identifiers must be AssetPack-native; promptpart identifiers may remain bounded proof inputs only when their rendered prompt text describes asset-pack runs, AssetPack synthesis artifacts, stored evidence, and Delivering Shippables through delivery mechanisms rather than teaching `deliverable` as the primary Bitcode object
- retained raw promptparts and promptpart-generation scripts should likewise describe the corridor in asset-pack-run, need-satisfaction, AssetPack synthesis artifact, stored-evidence, Shippable, and delivery-mechanism terms where compatibility filenames and identifiers still include `deliverable`; once a semantic mirror exists, the old family is removed rather than retained
...
- implementation-planning raw PromptParts with semantic mirrors use `promptpart_specific_agent_assetpackdiscoveryplanimplementation_*` filesystem/export names; `promptpart_specific_agent_deliverablesdiscimplplan_*` is removed after the prompt text is recut to Need requirements, written-asset synthesis, proof evidence, and validation criteria
...
- phase-purpose, setup-comprehension, finish-finalization, deliverables-system, implementation planning, PR-packaging, create-pull-request, and validation-ready-to-Finish promptparts must not teach deployment-ceremony-first or four-type implementation semantics where Bitcode now requires need-first AssetPack synthesis artifacts plus PR Shippables
