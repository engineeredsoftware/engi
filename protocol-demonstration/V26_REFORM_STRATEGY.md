...
- `deliverable` reform
  The retained corridor is now explicitly interpreted as asset-pack written-asset synthesis inside a Bitcode agentic pipeline-run corridor: each run satisfies a `need`, synthesizes stable written assets / asset-packs, and uses shipping-phase `deliverables` only as connected-interface delivery mechanisms. `need`, `writtenAssetType`, `writtenAssets`, `needSatisfactionCriteria`, delivery-mechanism mirrors, and asset-pack snapshots are progressively being carried alongside compatibility `deliverable*` naming, with registry-bearing runtime hydration on bare-execution entry, canonical `comprehend-need` module paths on the live setup corridor, semantic failure telemetry that survives route catch paths, local Prompt/PromptPart carriers on retained design/digest/validation PTRR agents, and an honest package-local typecheck boundary through the remaining retained support graph.
- `need-comprehension` prompt reform
  The retained generic tool package moved from the former task-comprehension boundary to a canonical need-comprehension package while keeping task-named APIs as compatibility wrappers; its DocCode prompts, raw PromptParts, README, placeholder primitive outputs, direct workspace dependencies, and no-emit package-local typecheck now describe and verify Bitcode need comprehension, written-asset requirements, asset-pack context, proof obligations, and delivery-mechanism limits.
