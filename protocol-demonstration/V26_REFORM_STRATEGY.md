...
- `need-comprehension` prompt reform
  The generic tool package is now a canonical need-comprehension package and removes noncanonical tool, prompt, primitive, schema, and raw PromptPart owners once need-first owners exist. Its individually defined tools, DocCode prompts, raw PromptParts, README, primitive outputs, direct workspace dependencies, TypeScript-only source, and no-emit package-local typecheck describe and verify Bitcode Need comprehension, written-asset requirements, asset-pack context, proof obligations, and delivery-mechanism limits. `NeedComprehensionToolset` is only a collection surface, and `packages/generic-agents/need-comprehension` is the separate TypeScript-only setup/pre-danger-wall PTRR agent that registers and composes those tools before risk admission.
