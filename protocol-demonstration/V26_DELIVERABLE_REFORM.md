...
- retained raw promptparts and promptpart-generation scripts should likewise describe the corridor in asset-pack-run, need-satisfaction, written-asset, and shipping-wrapper terms, even when compatibility filenames and identifiers still include `deliverable`; this includes phase-purpose, setup-comprehension, shipping-finalization, and deliverables-system promptparts, which should no longer teach PR-first or GA-1-first semantics where Bitcode now requires need-first asset-pack synthesis plus shipping wrappers
