...
- AssetPack substep doc-comment metadata must also be authored as Bitcode metadata: `current_version` may not carry pre-Bitcode lineage, and each `intent` must describe the need-first written-asset / asset-pack / proof / delivery-mechanism role rather than only restating bounded compatibility file names
