...
import { PROMPTPART_SPECIFIC_TOOL_DELIVERABLES_IMAGECOMPREHENSION_DOCCODETOOL_CAPABILITIES_ADDENDUM } from '@bitcode/prompts/raw_promptparts/specific/promptpart_specific_tool_deliverables_imagecomprehension_doccodetool_capabilities_addendum';
...
const OVERLAY: Prompt = (() => {
  ...
  p.set('capabilities:deliverables:addendum', PROMPTPART_SPECIFIC_TOOL_DELIVERABLES_IMAGECOMPREHENSION_DOCCODETOOL_CAPABILITIES_ADDENDUM as any, 5);
  return p;
})();

export const DP_IMAGE_TOOL_PROMPT: Prompt = (() => {
...
/**
 * @doc-code-tool
 * @prompt DP_IMAGE_TOOL_PROMPT
 */
export class DeliverablePipelineImageComprehensionTool extends Tool<typeof multimodalProcessingTool.use> {
  use = (input: Parameters<typeof multimodalProcessingTool.use>[0]) => multimodalProcessingTool.use(input);
}

export const deliverablePipelineImageComprehensionTool = new DeliverablePipelineImageComprehensionTool();
(deliverablePipelineImageComprehensionTool as any).name = 'deliverable-pipeline-image-comprehension-tool';
