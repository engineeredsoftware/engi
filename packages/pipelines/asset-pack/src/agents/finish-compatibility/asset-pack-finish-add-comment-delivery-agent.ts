...
const systemPrompt = (() => { 
  const p = new Prompt(); 
  p.set('agent:identity', createPromptPart('You add an AssetPack delivery comment to an issue or pull request.'));
...
export const AssetPackFinishAddCommentDeliveryAgent = factoryAgentWithPTRR<
  z.infer<typeof AddCommentInputSchema>,
  z.infer<typeof AddCommentOutputSchema>
>({
  name: 'finish:asset-pack-add-comment-delivery-agent',
  description: 'Deliver AssetPack evidence by adding a comment to an issue or pull request',
...
export default async function assetPackFinishAddCommentDeliveryAgent(input: any, execution: any) {
  ...
  const result = await AssetPackFinishAddCommentDeliveryAgent(args, execution);
  try {
    if (result?.url) {
      execution.store('finish', 'commentUrl', result.url);
      execution.store('shipping', 'commentUrl', result.url);
    }
    if (result?.title) {
      execution.store('finish', 'commentTitle', result.title);
      execution.store('shipping', 'commentTitle', result.title);
    }
  } catch {}
  return {
