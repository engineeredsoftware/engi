...
import { PROMPTPART_SPECIFIC_TOOL_ASSET_PACK_VIDEOCOMPREHENSION_DOCCODETOOL_CAPABILITIES_ADDENDUM } from '@bitcode/prompts/raw_promptparts/specific/promptpart_specific_tool_assetpack_videocomprehension_doccodetool_capabilities_addendum';
...
const OVERLAY: Prompt = (() => {
  ...
  p.set('capabilities:asset-pack:addendum', PROMPTPART_SPECIFIC_TOOL_ASSET_PACK_VIDEOCOMPREHENSION_DOCCODETOOL_CAPABILITIES_ADDENDUM as any, 5);
  return p;
})();

export const ASSET_PACK_VIDEO_TOOL_PROMPT: Prompt = (() => {
...
/**
 * @doc-code-tool
 * @prompt ASSET_PACK_VIDEO_TOOL_PROMPT
 */
export class AssetPackVideoComprehensionTool extends Tool<typeof multimodalProcessingTool.use> {
  use = (input: Parameters<typeof multimodalProcessingTool.use>[0]) => multimodalProcessingTool.use(input);
}

export const assetPackVideoComprehensionTool = new AssetPackVideoComprehensionTool();
(assetPackVideoComprehensionTool as any).name = 'asset-pack-video-comprehension-tool';
