...
> Disallow importing any *write-capable* Engi AI tool from code that is **not** part of an
...
1. Ensure `eslint-plugin-engi` is installed (present in the monorepo).
2. Enable the rule:
...
`__tests__/noWriteToolsOutsideConquer.test.ts` contains RuleTester specs verifying:
