...
import { createPromptPart as cpp } from '@bitcode/prompts/parts/PromptPart';
...
export function applyPromptOverlays(execution: Execution, prompt: PromptRegistry): void {
  // Evidence Documents overlay: list titles + details.
  try {
    const list = (execution as any).get?.('evidence_documents', 'list') || [];
    if (Array.isArray(list) && list.length) {
      const titles = list.map((u: any, i: number) => `(${i + 1}) ${u.title || 'Untitled'}`).join('\n');
      const details = list.map((u: any, i: number) => `### Evidence Document ${i + 1}: ${u.title || 'Untitled'}\n${(u.content || u.output || '').slice(0, 1200)}`).join('\n\n');
      prompt.set('preprocess:evidence_documents:list', cpp(titles as any));
      prompt.set('preprocess:evidence_documents:details', cpp(details as any));
    }
  } catch {}
