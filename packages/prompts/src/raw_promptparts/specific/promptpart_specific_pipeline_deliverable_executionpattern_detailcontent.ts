...
export const PROMPTPART_SPECIFIC_PIPELINE_DELIVERABLE_EXECUTIONPATTERN_DETAILCONTENT: PromptPart = 
  'SDIVF pattern: sequential(setup, repeat(sequential(discovery, implementation, validation), {until: score >= 0.9}), finish). Setup establishes repository and need context, discovery shapes the asset-pack synthesis approach, implementation produces written assets, validation verifies need satisfaction, and Finish emits connected-interface delivery mechanisms. Each phase stores results in namespaced execution storage. Phases retrieve previous results using execution.get(namespace, key).' as PromptPart;
