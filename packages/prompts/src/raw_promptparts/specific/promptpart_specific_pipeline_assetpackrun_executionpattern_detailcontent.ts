...
export const PROMPTPART_SPECIFIC_PIPELINE_ASSETPACKRUN_EXECUTIONPATTERN_DETAILCONTENT: PromptPart =
  'SDIVF pattern: sequential(setup, repeat(sequential(discovery, implementation, validation), {until: score >= 0.9}), finish). Setup establishes repository and need context, discovery shapes the AssetPack synthesis approach, implementation produces writtenAssetType = need-satisfaction-asset-pack plus AssetPack synthesis artifacts, validation verifies need satisfaction, and Finish emits connected-interface Shippables through delivery mechanisms. Each phase stores results in namespaced execution storage. Phases retrieve previous results using execution.get(namespace, key).' as PromptPart;
